...
WALL_CLOCK_BUDGET_SECONDS = 244.0  # v50: tighter budget from v49
...
TypeScript/React completeness cascade:
When adding a new React component, page, or feature — ALL of these must be present:
1. The component/page file itself
2. Route registration (React Router, Next.js pages, app router, etc.)
3. Navigation link (sidebar, navbar, menu — wherever users access it)
4. Any required state/context updates (auth guards, store entries, etc.)
...
Common Scope Failures (avoid all of these):
- Creating a new component/page/view without registering it in the router
- Adding a feature without updating navigation menus or sidebars
- Implementing a backend endpoint without wiring it to any frontend caller
- Updating a data model without migrating the database schema
- Adding a new utility/helper without updating any call sites that need it
- Fixing a function signature without updating all callers across the codebase
For BUGFIX tasks, scope to the failing code path and its direct callers — do not add new components or routes unless the bug is a missing feature.
...
# v50: Wiring gap gate — detect orphan presentation files not wired into router/nav
MAX_WIRING_NUDGES = 1
...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    wiring_nudges_used = 0  # v50: wiring gap gate
    ...
    def maybe_queue_refinement(assistant_text: str) -> bool:
        ...
        """
        nonlocal polish_turns_used, self_check_turns_used, syntax_fix_turns_used, test_fix_turns_used, coverage_nudges_used, criteria_nudges_used, hail_mary_turns_used, total_refinement_turns_used, must_edit_after_gap, must_edit_patch, gap_edit_nudges_used, deletion_nudges_used, wiring_nudges_used
        ...
        # v50: Wiring gap gate — detect orphan presentation files not wired
