...
def build_initial_user_prompt(issue: str, repo_summary: str, preloaded_context: str = "") -> str:
    ...
    # NEXT46 CHANGE 2: the filename-based detectors below search the REPO FILE
    # LIST (when CHANGE 1 supplied one for a multi-file repo) in addition to the
    # issue title, so DI/static-language hints fire on filenames (e.g.
    # src/di/Container.ts, *.ts, *.go) even when the title names neither. On a
    # small repo repo_summary is "" -> detect_text == issue (king-exact behaviour).
    ...
    # _is_js_integration_task) -- a seed-specific JS overfit, dropped with the
    ...
    # NEXT46 CHANGE 2: gate the static-language precision hint on `detect_text`
    # (issue + multi-file repo list) so `.ts`/`.go`/etc. extensions present in the
    # file list fire the "read the full owning implementation" hint even when the
    # TITLE names no language (T3 TS 9f, T8 TS DI 5f). The verb gates still read
    # `issue` (the actual task instruction) so a construction/feature task never
    # gets the precision hint.
    if (
        _PRECISION_FIX_RE.search(issue)
        and not _CONSTRUCT_VERB_RE.search(issue)
        and _STATIC_LANG_RE.search(detect_text)
    ):
        prompt = prompt + (
    ...
    # NEXT46 CHANGE 2: DI/Container read-first hint, fired off the FILE LIST.
    # T8 (src/di/Container.ts) loses 0.000 vs king 0.720 in some runs because no
    # DI vocabulary is in its title; with the repo file list available (multi-file
    # repo) the DI detector now fires and steers the agent to read the Container/
    # DI wiring before editing. On a small repo detect_text == issue so this only
    # fires when the TITLE itself names DI vocabulary (king-equivalent).
    if _CONTAINER_DI_RE.search(detect_text):
        prompt = prompt + (
            "\n[Dependency-injection repository: read the Container / DI wiring and "
            "the registered services FIRST, then focus your edit on the specific "
            "behavior the task asks for (e.g. error handling / reload logic) -- do "
            "not rewrite the DI registration itself.]"
        )
    return prompt
...
def solve(
    ...
    try:
        ...
        # returns "" for repos with < 5 files, so on small repos this is a no-op and
        # the prompt stays byte-identical to the live king (preserving the Next42
        # wins Next45's unconditional dump broke). On multi-file repos it gives the
        # model the file tree up front (cuts step-burn: T4/T18/T30) AND lets
        # build_initial_user_prompt()'s filename detectors fire (T8 DI). Reused for
        # the recovery run below.
        repo_summary = _repo_file_summary(repo_path)
        outcome = run_agent_loop(
            config=run_config,
            task=build_initial_user_prompt(issue, repo_summary, ""),
        )
        ...
        if not outcome.patch.strip():
            ...
            if remaining >= 60:
                ...
                # NEXT46 CHANGE 1: feed the same scoped repo file list into the
                # anti-collapse recovery prompt so the recovery run is not blind
                # on a multi-file repo (and a no-op on a small one).
                recovered = run_agent_loop(config=recovery_config, task=build_initial_user_prompt(recovery_prompt, repo_summary, ""))
