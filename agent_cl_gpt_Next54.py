#!/usr/bin/env python3
"""Next54 -- deeper timeout fix on the uid11 king base (86f697b7, 2555L).
Base is Next53 (king + 3 carries). stdlib only; zero new imports.

================================= NEXT54 (this version) =====================
The new king uid11 (SHA 86f697b7, 2555L) is DOMINANT: 9 defenses, 0 losses,
winning 20-22 decisive rounds per duel (typical margin -7 to -13). On-chain the
only close duel was 7217 (margin -2). Next53's 30-task gate (4W/7L/1T, killed at
12) showed our two FATAL losses were STILL `timeout_300s` -- T1 (C/C++ BUGFIX,
score 0.000) and T4 (Python API/ROUTE, score 0.000) -- the agent's own process
was KILLED at the 300s harness wall before its patch was collected. Next52/53
raised the CHILD-recovery thresholds (60->120/150) but those guards never touch
the MAIN solve loop's in-flight LLM call, which is exactly where T1/T4 died.

ROOT CAUSE (verified in run_agent_loop): the wall-clock guard fires only at the
TOP of each step; the next `model.query()` then runs with request_timeout=180s
and up to max_attempts=5 retries. A query that starts just under the wall can
run a full 180s round-trip and overrun the 300s harness wall -> killed mid-run
-> patch LOST (0.000). The longer uid11 king (347L more than the prior king)
burns budget faster, making this overrun MORE likely than under Next52.

THE EXACTLY TWO CHANGES vs Next53:
  * NEXT54 CHANGE 1 (PRIMARY -- the real timeout fix, NEW; no prior version had
    it): in `run_agent_loop`, before EVERY `model.query()`, bound the model's
    socket `request_timeout` by the remaining wall-clock budget (minus a 3s
    floor), and when budget is very tight (< 200s) drop `max_attempts` to 1 so
    the retry backoff cannot itself overrun. This GUARANTEES an in-flight LLM
    call can never push total elapsed past the harness wall -- structurally
    eliminating the T1/T4 main-loop timeout class that threshold-raising could
    not reach.
  * NEXT54 CHANGE 2 (further tune the child-recovery thresholds, per the build
    brief): anti-collapse 120 -> 150, secondary 150 -> 180, child reserve
    30 -> 45. The longer uid11 king needs a bigger margin; paired with CHANGE 1
    the child runs now fire only with a safe full-round-trip cushion.

UNCHANGED vs Next53 (do NOT regress): all 7 king hygiene functions
(_duplicate_method_error, _patch_corruption_error, _restore_mode_changes,
_scrub_backup_copies, _N_ARTIFACT_RE, _DOLLAR_PH_RE, _BACKUP_STEM_SUFFIXES) are
king-byte-identical and INTACT; SYSTEM_PROMPT / TASK_TEMPLATE /
...
Rust/Cargo fast-path hint (Next53 CHANGE 3) are carried verbatim; the verify-
...
def run_agent_loop(*, config: AgentRunConfig, task: str) -> AgentOutcome:
    ...
    for step in range(1, max(1, config.max_steps) + 1):
        if 0 < config.wall_clock_limit <= time.monotonic() - started:
            ...
            break
        # NEXT54 CHANGE 1 (PRIMARY -- the real timeout root cause fix). The
        # wall-clock guard above only fires at the TOP of a step, but the very
        # next `model.query()` carries `request_timeout=180s` and up to
        # `max_attempts=5` retries with backoff. A query that STARTS just under
        # the wall (e.g. at elapsed = wall_clock_limit - 1) can run a full 180s
        # round-trip and overrun the 300s HARNESS wall -> the process is killed
        # mid-flight -> our patch is LOST (score 0.000). This is exactly how
        # gate-next53 T1 (C/C++) and T4 (Python API/ROUTE) became `timeout_300s`
        # while the king scored: raising child-recovery thresholds (Next52/53)
        # never touched the MAIN loop's in-flight query, which is where these
        # two losses actually died. Fix: before EVERY query, bound the socket
        # request_timeout (and, when budget is very tight, the retry count) by
        # the remaining wall-clock budget minus a small safety floor, so an
        # in-flight call can NEVER push total elapsed past the harness wall.
        if config.wall_clock_limit > 0:
            remaining_budget = config.wall_clock_limit - (time.monotonic() - started)
            # Leave a 3s floor for diff collection + return after the call.
            allowed = max(15.0, remaining_budget - 3.0)
            if allowed < model.request_timeout:
                model.request_timeout = allowed
            # When there is not enough budget for even one full default
            # round-trip plus a retry, drop to a single attempt so the backoff
            # sleeps (up to ~20s each) cannot themselves overrun the wall.
...
def solve(
    ...
    try:
        ...
        if not outcome.patch.strip():
            ...
            # NEXT53 CHANGE 1 (PRIMARY -- timeout fix, ported from Next52):
            # threshold raised 60 -> 120. ROOT CAUSE (gate_next51 9/26=35% our
            # timeout rate; T17/T19/T25/T26 lost purely to timeout_300s while the
            # king scored): this child recovery run fires LATE (after the main
            # loop has burned ~most of the 280s budget) and starts a fresh agent
            # loop. The loop only checks `wall_clock_limit` at the TOP of each
            # step, but a single in-flight `model.query()` carries
            # request_timeout=180s. With only 60s of budget, one slow LLM
            # round-trip overruns the 300s HARNESS wall -> the process is killed
            # -> our patch is LOST (score 0.000). Raising to 120 ensures the
            # child run only fires when there is enough budget to safely contain
            # at least one full LLM round-trip plus the enlarged reserve.
            # NEXT54 CHANGE 2: threshold raised 120 -> 150. The new king (uid11)
            # is 347 lines LONGER than the prior king (2555L vs 2208L), so each
            # of its solve/recovery turns executes more code and consumes more
            # wall-clock budget; the child anti-collapse run therefore fires even
            # closer to the 300s harness edge than under Next53. 150 (paired with
            # the per-query request_timeout bound from CHANGE 1) keeps a real
            # margin for a full LLM round-trip plus the enlarged 45s reserve.
            if remaining >= 150:
                ...
                recovery_config = AgentRunConfig(
                    ...
                    max_observation_chars=MAX_OBSERVATION_CHARS,
                    max_log_chars=MAX_TOTAL_LOG_CHARS,
                    # NEXT54 CHANGE 2: reserve raised 30 -> 45. The child loop
                    # stops 45s before the harness wall, so even with the
                    # per-query request_timeout bound (CHANGE 1) there is a
                    # comfortable cushion for diff collection + return. The longer
                    # uid11 king burns budget faster, so a bigger reserve is the
                    # safe choice.
                    wall_clock_limit=remaining - 45.0,
                    issue_text=issue,
                )
        ...
        try:
            remaining = WALL_CLOCK_LIMIT_SECONDS - (time.monotonic() - started)
            # NEXT53 CHANGE 1 (PRIMARY -- timeout fix, ported from Next52):
            # threshold raised 60 -> 150. This secondary recovery is the LAST
            # child run before the polish tail, so it fires closest to the 300s
            # harness edge and is the prime timeout culprit. It does a
            # `git reset --hard` + `clean -fd` and then runs a FULL fresh agent
            # loop (up to 18 steps). With only 60s budget, the first
            # `model.query()` (request_timeout=180s) can run long past the 300s
            # harness wall -> the process is killed mid-run -> the patch we
            # already had is LOST (exactly how T17/T19/T25/T26 became 0.000
            # losses while king scored). 150 guarantees a real budget for a safe
            # LLM round-trip plus the enlarged 30s reserve; below 150 we keep the
            # patch we already have rather than risk a fatal timeout.
            # NEXT54 CHANGE 2: threshold raised 150 -> 180. This secondary
            # recovery is the LAST child run before the (disabled) polish tail,
            # so it fires closest to the 300s harness edge and is the prime
            # timeout culprit. With the longer uid11 king and the per-query
            # request_timeout bound (CHANGE 1) both in play, 180 guarantees a
            # safe full round-trip plus the 45s reserve; below 180 we keep the
            # patch we already have rather than risk a fatal timeout.
            if remaining >= 180:
                # Check if the current patch is broken. NOTE: the new king (uid11)
                # base HAS `_patch_corruption_error`, so "corruption" is a valid
                # repair reason here -- keep it in the trigger set (do NOT regress
                # to the naruto-era empty/syntax-only check).
                ...
                if reason is not None and reason[0] in ("empty", "syntax", "corruption"):
                    ...
                    recovery_config = AgentRunConfig(
                        ...
                        max_observation_chars=MAX_OBSERVATION_CHARS,
                        max_log_chars=MAX_TOTAL_LOG_CHARS,
                        # NEXT54 CHANGE 2: reserve raised 30 -> 45.
                        wall_clock_limit=remaining - 45.0,
                        issue_text=issue,
                    )
                    recovered = run_agent_loop(config=recovery_config, task=build_initial_user_prompt(recovery_prompt, "", ""))
