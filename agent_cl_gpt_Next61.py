#!/usr/bin/env python3
"""Next61 -- BASE = the LIVE NEW KING agent.py (unarbos/ninja SHA 395096dfe1ec,
2789 lines), copied byte-for-byte, then ONE surgical, prompt-only, zero-step-cost
quality lever added. stdlib only; zero new imports; no sampling params.

================================ NEXT61 (this version) =====================
CRITICAL CONTEXT (2026-06-25): the current king (uid=125, SHA 395096) is a
DESCENDANT OF OUR OWN LINEAGE -- sync_king.sh fetched it from unarbos/ninja and
it IS our "Next41 + Next42 content-hints + secondary-recovery + 280s wall-clock"
code. This is why uid=167 shows a 100% LIVE TIE RATE (156 rounds, 0 decisive
wins, 0 losses): we have been fighting a near-identical copy of ourselves, so the
judge scores both patches within the 2% decisive band -> tie. A king-pure agent
CANNOT dethrone a king-pure king; it ties forever. To win (win_margin is now 6,
not 3), we must be DECISIVELY better on the loss categories WITHOUT adding any
timeout risk (timeout = empty patch = 0.000 = auto-loss, our single worst mode).

WHAT THE NEW KING ALREADY HAS (all INHERITED verbatim -- do NOT regress):
  * Next42 content hints: _CONTAINER_DI_RE (DI/Container) + _LARGE_REPO_RE
    (pipeline/router/14-files) in _integration_hints().
  * The "NEW CRITICAL STRONGEST PART": a SECONDARY recovery run that
    `git reset --hard` + `git clean -fd` and re-solves from clean when the patch
    is still empty/syntax-broken/corrupt after the verify-repair gate.
  * 280s WALL_CLOCK_LIMIT + 40s command_timeout + per-call request_timeout bound
    (the reliability stack that prevents most SIGKILL-at-300s empty patches).
  * king-pure _integration_hints, _FILE_TARGET_HINT, _API_TASK_HINT, large-repo
    early-focus, static-language precision read-before-edit, verify-repair gate,
    anti-collapse recovery, _sanitize_patch, robust action parser.

GATE EVIDENCE that drives the ONE change (Next59 16W-9L-1T margin+7 PASS; Next60
15W-13L-2T margin+2 FAIL, both vs OLD king 86f697 2555L):
  * The dominant FIXABLE, ZERO-TIMEOUT-RISK loss class is CLOSE JavaScript BUGFIX
    losses where OUR cursor-sim telemetry is ABOVE the king yet the judge edges
    us by ~0.06: Next59 T20 ours 0.720 vs king 0.780, T22 ours 0.820 vs king
    0.880. JavaScript is the single highest-volume language (7 tasks/gate) and
    the king's static-language read-before-edit branch DELIBERATELY EXCLUDES
    .js/.jsx (_STATIC_LANG_RE has no JS), so JS BUGFIX rounds get NO
    read-before-edit discipline at all -- they over-patch callers without first
    reading the full owning implementation, losing the last ~0.06.
  * Next60's regression (margin +7 -> +2) came from a budget/timeout change that
    ADDED challenger timeouts (T4 API 0.000, T8 DI 0.000, T12 PHP 0.000) -- we do
    NOT repeat that. Next61 keeps the new king's 280s reliability stack untouched
    and makes the JS change PROMPT-ONLY (zero extra steps, zero timeout risk).

THE EXACTLY ONE CHANGE vs the new king (NEXT61 CHANGE 1, prompt-only):
  Add a narrow `_JS_LANG_RE` (matches .js/.jsx/.mjs/.cjs and the word
  'javascript'; EXCLUDES .ts/.tsx and .json) and a THIRD, mutually-exclusive
  read-before-edit branch in build_initial_user_prompt(): it fires ONLY for a
  JavaScript task with NO construction verb (a true BUGFIX) when NEITHER the
  precision-fix static branch NOR an API branch already fired -- so no prompt
  ever stacks two read-before-edit blocks. The nudge text is identical to the
  king's proven static-language read-before-edit hint. This extends a
  gate-validated king mechanism to the one high-volume language it excludes,
  targeting the documented close JS BUGFIX losses, at zero step cost and zero
  timeout risk. SYSTEM_PROMPT / TASK_TEMPLATE / render_observation / solve() /
  the entire reliability stack remain BYTE-IDENTICAL to the king.
...
# NEXT61 CHANGE 1: narrow JavaScript detector for the JS BUGFIX read-before-edit
# branch. Matches .js/.jsx/.mjs/.cjs and the word 'javascript', but EXCLUDES
# TypeScript (.ts/.tsx) and .json (no negative lookbehind needed: .ts/.tsx are
# matched by _STATIC_LANG_RE which fires its own read-before-edit branch first,
# and the new branch is gated to fire only when no static branch already fired).
# JavaScript is the single highest-volume language (7 tasks/gate) and the king's
# _STATIC_LANG_RE read-before-edit hint deliberately excludes it, so JS BUGFIX
# rounds get NO read discipline -- exactly the close losses (Next59 T20 0.720 vs
# 0.780, T22 0.820 vs 0.880) this branch targets.
...
def build_initial_user_prompt(issue: str, repo_summary: str, preloaded_context: str = "") -> str:
    ...
    # NEXT61 CHANGE 1: JavaScript BUGFIX read-before-edit branch. Fires ONLY for a
    # JS task (_JS_LANG_RE) with NO construction verb (a true BUGFIX, not a new
    # build) when NEITHER the static-language read branch above NOR the API/route
    # branch already fired -- so no prompt ever stacks two read-before-edit
    # blocks. Same nudge text as the proven static-language branch. Zero step
    # cost (prompt-only) -> zero added timeout risk. Targets the documented close
    # JS BUGFIX losses where our cursor-sim was ABOVE the king yet the judge
    # edged us by ~0.06 because we over-patched callers without first reading the
    # full owning implementation.
    if (
