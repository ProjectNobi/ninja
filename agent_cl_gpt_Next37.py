#!/usr/bin/env python3
"""Next37 -- base Next36 + CHANGE1(pipeline-exclusion) + CHANGE2(FEATURE-minimal)
+ CHANGE3(TS-style-hint). stdlib only; zero new imports.
...
def _integration_hints(issue: str) -> List[str]:
    ...
    if _GO_LANG_RE.search(issue) and len(_FILE_EXT_RE.findall(issue)) > 7:
        hints.append(
            ...
            "touching integration test files unless the fix explicitly requires "
            "it -- concentrate on the core logic files first."
        )
