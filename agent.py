...
DEFAULT_MAX_STEPS = int(os.environ.get("AGENT_MAX_STEPS", "18"))  # CL-GPT-v1: 18 steps
...
MAX_WIRING_CHECK_TURNS = 1  # CL-GPT-v1: one wiring check per solve
...
def _messages_for_request(messages: List[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    note = {
        ...
        "content": (
            ...
            "the most complete fix you can within the remaining budget.]"
        ),
    }
...
Discipline:
- Work directly in the repository. Make a COMPLETE fix: address the ROOT CAUSE
  (not just the symptom), update every affected file, and update all callers or
  consumers of any changed function/class/interface. Missing required changes
  hurt more than thorough changes. A fix touching 6+ files is normal for
  non-trivial tasks.
...
def build_wiring_check_prompt(patch: str) -> str:
    """Wiring verification pass -- checks for unupdated callers/consumers."""
    truncated = patch if len(patch) <= 3000 else patch[:1500] + "\n...[truncated]...\n" + patch[-1000:]
    return (
        "Wiring-check pass. Review your patch for integration completeness:\n\n"
        "Your patch so far:\n```diff\n"
        f"{truncated}\n```\n\n"
        "Check: Are there any callers, importers, or consumers of the code you "
        "changed that you have NOT yet updated? For example:\n"
        "  - Functions that call a renamed/modified function\n"
        "  - Modules that import a renamed/moved symbol\n"
        "  - Tests that exercise the changed code path\n"
        "  - Type definitions or interfaces that reference changed types\n"
        "  - Database migrations if schema was modified\n\n"
        "If you find any remaining callers that need updating, emit the fix "
        "commands now, then end with <final>summary</final>.\n"
        "If the patch is already fully wired, respond exactly:\n<final>OK</final>"
    )
...
def solve(
    ...
    wiring_check_turns_used = 0  # CL-GPT-v1: wiring check
    ...
    try:
        ...
        for step in range(1, max_steps + 1):
            ...
            if not commands:
                if final is not None:
                    ...
                    if junk and polish_turns_used < MAX_POLISH_TURNS:
                        ...
                        continue
                    if patch.strip() and wiring_check_turns_used < MAX_WIRING_CHECK_TURNS:
                        wiring_check_turns_used += 1
                        logs.append("\nWIRING_CHECK_TURN_QUEUED")
                        messages.append({"role": "assistant", "content": response_text})
                        messages.append({"role": "user", "content": build_wiring_check_prompt(patch)})
                        continue
            ...
            wiring_check_pending = False  # CL-GPT-v1
            if final is not None and get_patch(repo).strip():
                ...
                if junk and polish_turns_used < MAX_POLISH_TURNS:
                    ...
                    polish_turns_used += 1
                    logs.append("\nPOLISH_TURN_QUEUED:\n" + junk)
                elif wiring_check_turns_used < MAX_WIRING_CHECK_TURNS:  # CL-GPT-v1: wiring check
                    wiring_check_pending = True
                    wiring_check_turns_used += 1
                    logs.append("\nWIRING_CHECK_TURN_QUEUED")
            ...
            if observations:
                ...
                elif wiring_check_pending:  # CL-GPT-v1
                    observation_text += "\n\n" + build_wiring_check_prompt(get_patch(repo))
            ...
            elif wiring_check_pending:
                messages.append({"role": "user", "content": build_wiring_check_prompt(get_patch(repo))})
