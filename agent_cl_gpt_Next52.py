#!/usr/bin/env python3
"""Next52 -- timeout-rate fix on the naruto (king) base. Base is king_agent.py
(naruto, 2208 lines) + Next51's two harmless riders (injection neutralizer, Rust
hint). stdlib only; zero new imports.

================================= NEXT52 (this version) =====================
Gate evidence (gate_next51_20260624_090837.log, killed 24/30 at 9W-11L-6T,
margin -2): our TIMEOUT rate was 9/26 (35%) vs the king's 8/26 (31%) on the SAME
tasks. Four LOSSES were PURE timeout where the king did NOT time out and scored:
T17 [FEATURE] C/C++ (king 0.300), T19 [API/ROUTE] TypeScript (king 0.450),
T25 [BUGFIX] TypeScript (king 0.720), T26 [UPDATE] TypeScript (king 0.450). Five
rounds (T4/T10/T12/T14/T16) were mutual-timeout forced TIES. The seven clean
losses (T1/T6/T9/T13/T21/T22/T24) were close-margin judge calls (T22 we 0.840 vs
king 0.880, cursor-sim us 0.350 > king 0.239) -- within judge variance on
near-equal patches, not a fixable structural defect, so they are intentionally
NOT touched (every prior JS-specific hint overfit and regressed the broad pool).

ROOT CAUSE OF THE EXTRA TIMEOUTS: the two child RECOVERY runs in solve() fired
at `remaining >= 60`. They start a FRESH agent loop (up to 18 steps); that loop
only checks `wall_clock_limit` at the TOP of each step, but a single in-flight
`model.query()` carries `request_timeout=180s`. Fired with only ~60s of budget
near the 280s mark, one slow LLM round-trip overruns the 300s HARNESS wall ->
the process is killed mid-run -> the patch we already had is LOST (score 0.000).
This latent bug is present byte-identically in the king too; it manifests
stochastically on whichever late-pool tasks trigger a recovery run -- on this
gate it bit us on T17/T19/T25/T26.

THE EXACTLY TWO CHANGES vs the king base (both in solve(), timeout only):
  * NEXT52 CHANGE 1 -- ANTI-COLLAPSE (empty-patch) RECOVERY threshold raised
    `remaining >= 60` -> `remaining >= 120`, and the child `wall_clock_limit`
    reserve raised `remaining - 10.0` -> `remaining - 30.0`.
  * NEXT52 CHANGE 2 -- SECONDARY RECOVERY (broken/empty patch, with git reset)
    threshold raised `remaining >= 60` -> `remaining >= 150` (it fires LAST,
    closest to the 300s edge -> prime culprit), child reserve `- 10.0` -> `- 30.0`.
Below these thresholds we KEEP the patch we already have instead of risking a
fatal mid-query timeout that would zero the whole round. The verify-repair gate
(VERIFY_REPAIR_MIN_BUDGET_SECONDS=45, WALL_CLOCK_RESERVE_SECONDS=10) is left
king-byte-identical to minimise regression risk.

PRESERVED EXACTLY (do NOT regress): SYSTEM_PROMPT, TASK_TEMPLATE,
render_observation (all three king-byte-identical, fingerprint-verified vs
king_agent.py), naruto's secondary-recovery mechanism (intact, threshold-fixed),
the aggressive adopt=True logic in the repair/recovery paths, _sanitize_patch,
the robust action parser. Next51's two riders are kept because they do not hurt:
the injection neutralizer (`_neutralize_injection_in_added_line`, runs once in
_sanitize_patch -- cheap in-memory string ops, NOT in any hot loop) and the Rust
/Cargo hint (`_is_rust_task`, fires only on Rust repos, rare in pool). stdlib
...
# ============================================================
# NEXT51 CHANGE 1 (injection-guard auto-fail net -- ported from Next49/Next50)
# ============================================================
...
# error the original patch is returned unchanged.
        ...
        for ln in lines:
            ...
            if body.startswith("+") and not body.startswith("+++"):
                ...
                if _line_is_autofail(added_content):
                    removed_any = True
                    continue
                # NEXT51 CHANGE 1: neutralize injection-guard phrases in place
...
# NEXT51 CHANGE 2 (Rust/Cargo fast-path hint -- ported from Next50). A NARROW
# detector that matches ONLY unambiguous Rust markers (.rs file, Cargo.toml, a
# `cargo <subcommand>`, or the datafusion / arrow-rs crate names). It does NOT
# match `.tsx`/`.ts`/`.js`/`.py`/`.go`/`.php`/`.cpp`, so the king's 93%+ React/TS
# wins are untouched. `.rs\b` will not match `.rsx`; `impl`/`fn main` prose is
# intentionally NOT a trigger (too risky). On the live DataFusion tasks the king
# times out browsing the tree before fixing (0% / 3.2% WR); this hint redirects
# the agent to `cargo check`-driven navigation (compiler error as map).
...
def _is_rust_task(issue: str) -> bool:
    """NEXT51 CHANGE 2: True only when the task carries an unambiguous Rust/Cargo
...
def build_initial_user_prompt(issue: str, repo_summary: str, preloaded_context: str = "") -> str:
    ...
    # NEXT51 CHANGE 2: Rust/Cargo fast-path hint (zero step cost). Fires FIRST
...
def solve(
    ...
    try:
        ...
        if not outcome.patch.strip():
            ...
            # NEXT52 CHANGE 1 (PRIMARY -- timeout fix): threshold raised 60 -> 120.
            # ROOT CAUSE (gate_next51 9/26=35% our timeout rate vs king 8/26=31%,
            # losing T17/T19/T25/T26 purely to timeout_300s while king scored):
            # this child recovery run fires LATE (after the main loop has burned
            # ~most of the 280s budget) and starts a fresh agent loop. The loop
            # only checks `wall_clock_limit` at the TOP of each step, but a single
            # in-flight `model.query()` carries request_timeout=180s. With only
            # 60s of budget, one slow LLM round-trip overruns the 300s HARNESS
            # wall -> the process is killed -> our patch is LOST (score 0.000).
            # Raising to 120 ensures the child run only fires when there is enough
            # budget to safely contain at least one full LLM round-trip plus the
            # enlarged 30s reserve, so a mid-query overrun no longer crosses 300s.
            if remaining >= 120:
                ...
                recovery_config = AgentRunConfig(
                    ...
                    max_observation_chars=MAX_OBSERVATION_CHARS,
                    max_log_chars=MAX_TOTAL_LOG_CHARS,
                    # NEXT52 CHANGE 1: reserve raised 10 -> 30. The child loop
                    # stops earlier relative to the harness wall, so an in-flight
                    # LLM call (up to 180s) is far less likely to push total
                    # elapsed past the 300s harness limit.
                    wall_clock_limit=remaining - 30.0,
                    issue_text=issue,
                )
        ...
        try:
            remaining = WALL_CLOCK_LIMIT_SECONDS - (time.monotonic() - started)
            # NEXT52 CHANGE 2 (PRIMARY -- timeout fix): threshold raised 60 -> 150.
            # This secondary recovery is the LAST child run before the polish
            # tail, so it fires closest to the 300s harness edge and is the prime
            # timeout culprit. It does a `git reset --hard` + `clean -fd` and then
            # runs a FULL fresh agent loop (up to 18 steps). With only 60s budget,
            # the first `model.query()` (request_timeout=180s) can run long past
            # the 300s harness wall -> the process is killed mid-run -> the patch
            # we already had is LOST (this is exactly how T17/T19/T25/T26 became
            # 0.000 losses while king scored). 150 guarantees a real budget for a
            # safe LLM round-trip plus the enlarged 30s reserve; below 150 we keep
            # the patch we already have rather than risk a fatal timeout.
            if remaining >= 150:
                ...
                if reason is not None and reason[0] in ("empty", "syntax"):
                    ...
                    recovery_config = AgentRunConfig(
                        ...
                        max_observation_chars=MAX_OBSERVATION_CHARS,
                        max_log_chars=MAX_TOTAL_LOG_CHARS,
                        # NEXT52 CHANGE 2: reserve raised 10 -> 30 (see CHANGE 1).
                        wall_clock_limit=remaining - 30.0,
                        issue_text=issue,
                    )
                    recovered = run_agent_loop(config=recovery_config, task=build_initial_user_prompt(recovery_prompt, "", ""))
