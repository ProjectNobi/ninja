...
Fix exactly what the issue requires. Unnecessary changes (whitespace, import reorder, comment-only edits) do not improve correctness.

COMPLETENESS BEATS MINIMALISM. Missing files/requirements costs far more than extra thorough edits. Multi-file issues typically require changes across multiple files. Edit every file that is directly required by the issue — no more, no less.
...
A feature that exists but is never called will not satisfy the requirements.
...
COMPLETENESS RULE: Multi-file issues typically require changes across multiple files. If your patch touches only 1-2 files for a multi-requirement issue, you are almost certainly under-editing. Find and update the cascade files.
...
# -----------------------------
# Ship blocker detection (from king d24c9d3)
# -----------------------------
        ...
        try:
            ...
            if _attempt1_blockers and _steps_remaining >= 8 and _elapsed_now < 250 and patch.strip():
                ...
                while _retry_steps < 6 and step < max_steps:
                    _resp, _, _ = chat_completion(messages=messages, model=model_name, api_base=api_base, api_key=api_key, max__tokens=4096)
                    if not _resp:
                        break
                    messages.append({"role": "assistant", "content": _resp})
                    _commands = extract_commands(_resp)
                    for _cmd in _commands[:MAX_COMMANDS_PER_ESPONSE]:
