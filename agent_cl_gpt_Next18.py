#!/usr/bin/env python3
"""SN66 Next18 challenger -- Next17 with three targeted duel-6862 fixes.

Next18's three (and ONLY three) changes versus Next17:
  * CHANGE 1: remove Next17's `remaining_steps <= 6` "stop exploring" pressure
    tier (it fired too early on complex multi-requirement tasks, forcing
    premature partial submits -- the 5 collapse rounds 0.20-0.45 vs king 0.85).
    Keep ONLY the `<= 3` FINAL tier, strengthened to also demand the regression
    test in the last patch.
  * CHANGE 2: the SYSTEM_PROMPT rider's action-first line is reframed from
    "make your first edit within 4 steps" (read as "you only have 4 steps") to
    "start your first edit by step 4 at the latest -- then handle EVERY
    requirement ... and include a focused regression test before submitting".
    Adds the missing regression-test reminder to the rider.
  * CHANGE 3: the `extract_criteria` generic fallback now fires only when the
    extracted checklist is truly EMPTY (len == 0), not when len < 2. A task
    with even ONE clear criterion keeps its focused guidance and is no longer
    diluted by the two generic completeness hints.

Everything else (robust parser, _sanitize_patch, king SYSTEM_PROMPT body, king
TASK_TEMPLATE, criteria injection, guards, verify-repair gate, solve contract)
is kept verbatim from Next17.

---- Next17 provenance (unchanged) ----
Next17 == a flat, single-file re-base on the current king
(unarbos/ninja 16e2f934). Next17 == Next16 (robust fence parser +
_sanitize_patch + 3-line rider, ALL KEPT) plus exactly THREE targeted
step-budget / completeness fixes motivated by the Next16 gate (W3/L5/T1, 37.5%
WR), where every loss with a 0.000 score was a TypeScript TIMEOUT (tasks 3, 8,
9) -- we were still exploring large multi-file repos at step 15 while the king's
minimal loop had already fixed and submitted.
...
Start your first edit by step 4 at the latest -- then handle EVERY requirement the task states and include a focused regression test before submitting.
"""
...
def render_observation(*, returncode: int, output_text: str, remaining_steps: int) -> str:
    # NEXT18 CHANGE 1: drop the Next17 <= 6 "stop exploring" tier. In duel 6862
    # that early-pressure tier triggered before the agent had read every file a
    # complex multi-requirement task needs, forcing a premature partial submit
    # (the 5 collapse rounds, challenger 0.20-0.45 vs king 0.85). Keep ONLY the
    # <= 3 FINAL tier, but strengthen it to also demand the regression test in
    # the last patch (the missing 0.10 on the near-miss rounds).
    if remaining_steps <= 3:
        remaining_note = (
            f"[{remaining_steps} step(s) left -- FINAL. Apply your best patch now, "
            f"include the regression test, then echo {COMPLETION_SENTINEL}. Do not "
            f"read more files.]"
        )
    else:
        remaining_note = ""
...
def extract_criteria(issue: str) -> List[str]:
    ...
    for hint in _integration_hints(issue):
        if hint not in out:
            out.append(hint)
    # NEXT18 CHANGE 3: fire the generic fallback checklist ONLY when truly empty
    # (len == 0), not when len < 2. In duel 6862 the two generic hints diluted
    # focused narrow-fix tasks that already carried one clear criterion, nudging
    # the model to over-edit. A task with even ONE extracted criterion now keeps
    # its own focused guidance untouched; only a checklist that would otherwise
    # be completely blank gets the two generic backfills.
    if len(out) == 0:
        for fallback in (
