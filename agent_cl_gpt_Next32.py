#!/usr/bin/env python3
"""SN66 Next32 challenger -- POLISH-OPPORTUNITY + COLLAPSE + T8-DI FIX. Base is
Next31 + EXACTLY THREE surgical changes derived from TWO duels (7029 + 7031) and
the gate-31 partial. stdlib only; zero new imports.
...
# NEXT32 CHANGE 2 (collapse second-recovery, duel-7031): count the actual
# changed (+/-) lines in a unified diff, ignoring the +++/--- file headers. Used
# to detect a suspiciously THIN recovered patch (the duel-7031 R37=0.200,
# R39=0.300, R42=0.200 collapses were single-line recoveries that did not
# address the core requirement).
...
def solve(
    ...
    try:
        ...
        if not outcome.patch.strip():
            ...
            if remaining >= 60:
                ...
                if recovered.patch.strip():
                    outcome = recovered
                    # NEXT32 CHANGE 2 (second recovery on THIN patch, duel-7031):
                    # duel-7031 still had R37=0.200, R39=0.300, R42=0.200
                    # collapses despite Next31's recovery -- the recovered patch
                    # was non-empty but suspiciously SHORT (a single-line change
                    # that the judge sees but scores ~0.2 because it does not
                    # address the core requirement). If the recovered diff has
                    # fewer than 5 real changed lines AND >= 45s remain, fire ONE
                    # broader second recovery that asks for the full fix (not a
                    # single minimal edit). max_steps=8 keeps it bounded.
                    second_remaining = WALL_CLOCK_LIMIT_SECONDS - (time.monotonic() - started)
                    if _patch_change_lines(recovered.patch) < 5 and second_remaining >= 45:
                        second_prompt = _recovery_prompt(issue).replace(
                            "make ONE minimal, targeted edit",
                            "make the necessary changes to fully address the core requirement",
                        )
                        second_config = AgentRunConfig(
                            repo_dir=repo_path,
                            model_name=model_name,
                            base_url=base_url,
                            auth_token=proxy_token,
                            ...
                            max_observation_chars=MAX_OBSERVATION_CHARS,
                            max_log_chars=MAX_TOTAL_LOG_CHARS,
                            wall_clock_limit=second_remaining - 10.0,
                            issue_text=issue,
                        )
                        second_recovery = run_agent_loop(
                            config=second_config,
                            task=build_initial_user_prompt(second_prompt, "", ""),
                        )
                        # Adopt the second recovery only if it produced a
                        # SUBSTANTIVELY larger patch (more changed lines) than the
                        # thin first one -- never regress to a smaller diff.
                        if (second_recovery.patch.strip()
                                and _patch_change_lines(second_recovery.patch)
                                > _patch_change_lines(recovered.patch)):
                            outcome = second_recovery
        ...
        try:
            ...
            # NEXT32 CHANGE 1 (polish gate relaxed 90 -> 60, duel-7029+7031):
            # _build_polish_task is VERIFIED byte-identical to king hashirama's
            # (SHA 53bca97c) version -- no text drift. The 0.850 vs 0.950 gap is
            # NOT prompt wording but polish OPPORTUNITY: in duel-7031 the king
            # hit 0.950 on 13/22 of our loss rounds where we scored 0.850 (good
            # but un-polished). The king ALWAYS runs its polish; our >= 90s gate
            # suppressed it on many of those rounds. Relax to >= 60s so the
            # polish fires more often (closing 0.850 -> 0.950) while still
            # preventing the R48-type rushed-polish collapse (which only happens
            # with almost no time left). 60s is enough for one tight polish run.
            ...
            if polish_reason is None and can_repair and outcome.patch.strip() and time_remaining >= 60:
                kind, message = (
                    "polish",
