...
WALL_CLOCK_BUDGET_SECONDS = 244.0
...
MAX_REQ_VERIFY_TURNS = 1   # v41: closed-loop requirement verification gate
MAX_WIRING_NUDGES = 1      # v49: detect orphan presentation files not wired into router/nav
_REQ_VERIFY_MIN_PATCH_LINES = 3  # only run req verify if patch has >= this many substantive lines
...
def chat_completion(
    ...
    # OpenRouter provider routing: prefer direct MiniMax endpoint for native KV-cache.
    # The validator proxy rewrites model+sampling but may pass OpenRouter-specific keys.
    # allow_fallbacks=True means graceful degradation if direct routing fails.
    try:
        _api_base_lower = (base or "").lower()
        if "openrouter" in _api_base_lower or "openrouter" in (model_name or "").lower():
            payload["provider"] = {
                "order": ["MiniMax"],
                "allow_fallbacks": True,
            }
    except Exception:
        pass  # never fail on routing hint errors
...
# --- JS/TS import-concat repair (ported from king PR#1576) ---
...
def _filter_out_of_scope_files(
    ...
    # Build issue keywords (meaningful tokens ≥4 chars)
    issue_tokens = set(re.findall(r'[a-zA-Z_][a-zA-Z0-9_]{3,}', issue_text.lower()))
    if not issue_tokens:
        return patch

    # Normalise preloaded top-5 paths for substring match
    ...
    for block in blocks:
        ...
        # Always keep new-file creations (--- /dev/null) — issue may not name the new file
        if '--- /dev/null' in block:
            kept.append(block)
            continue

        # Is this file in the top-5 preloaded context? → always keep
        in_preloaded = any(
        ...
        # Issue keyword overlap check (path-level only — cheap and reliable)
        ...
        if overlap:
            ...
            removed_count += 1
            # Don't append — this file is a scope-filter candidate

    if removed_count == 0:
        return patch  # Nothing filtered

    candidate = ''.join(kept)
    if not candidate.strip():
        return patch  # Rollback: never produce an empty patch
...
def build_preloaded_context(repo: Path, issue: str) -> Tuple[str, List[str]]:
    ...
    for file_idx, relative_path in enumerate(files[:MAX_PRELOADED_FILES]):
        ...
        snippet = _read_context_file(repo, relative_path, per_file_budget, issue_text=issue)
...
_NEEDLE_CAMEL_RE = re.compile(r"[A-Z][a-z]+(?:[A-Z][a-z]+)+")
_NEEDLE_SNAKE_RE = re.compile(r"[a-z][a-z0-9]*(?:_[a-z0-9]+)+")
_NEEDLE_BACKTICK_RE = re.compile(r"`([^`]{3,60})`")


def _preload_needles(issue: str) -> List[str]:
    """Extract key identifiers/terms from issue for needle search."""
    needles: List[str] = []
    seen: set = set()
    for m in _NEEDLE_BACKTICK_RE.finditer(issue):
        term = m.group(1).strip()
        if term and len(term) >= 3 and term.lower() not in seen:
            seen.add(term.lower())
            needles.append(term)
    for m in _NEEDLE_CAMEL_RE.finditer(issue):
        term = m.group(0)
        if term.lower() not in seen:
            seen.add(term.lower())
            needles.append(term)
    for m in _NEEDLE_SNAKE_RE.finditer(issue):
        term = m.group(0)
        if term.lower() not in seen:
            seen.add(term.lower())
            needles.append(term)
...
def _extract_relevant_regions(text: str, needles: List[str], window: int = 40, max_chars: int = 0) -> str:
    """Return windowed line ranges (\u00b1window) around lines matching needles.

    Pure-Python fallback for needle-based file reading when rg is unavailable
    or finds no hits. Only useful for large files where head/tail truncation
    loses critical middle sections.
    """
    if not needles:
        return ""
    ...
    for hit in hit_set:
        ...
        end = min(len(all_lines) - 1, hit + window)
        for i in range(start, end + 1):
            region_set.add(i)
    sorted_idxs = sorted(region_set)
    out_lines: list = []
    prev_idx = -1
    for idx in sorted_idxs:
        if prev_idx >= 0 and idx > prev_idx + 1:
            out_lines.append(f"... [lines {prev_idx+2}\u2013{idx} omitted] ...")
        out_lines.append(f"{idx+1:4d}: {all_lines[idx]}")
        prev_idx = idx
    focused = "\n".join(out_lines)
    if max_chars and len(focused) > max_chars:
        return _truncate(focused, max_chars)
    return focused
...
def _read_context_file(
    ...
    """Read a tracked file, returning the most relevant region.

    When the file fits within max_chars, returns full content unchanged.
    When issue_text is given and the file is large, uses rg to find lines
    containing issue identifier tokens and returns context windows around
    each hit. Falls back to pure-Python needle search, then to head-of-file
    truncation when no issue_text is provided or no hits found.
    """
    ...
    if len(text) <= max_chars:
        return text
    if issue_text and len(text) > max_chars:
        focused = _read_focused_region(repo, relative_path, text, issue_text, max_chars)
        if focused:
            return focused
        # Pure-Python needle fallback for large files (>300 lines)
        line_count = text.count("\n") + 1
        if line_count > 300:
            needles = _preload_needles(issue_text)
            if needles:
                needle_focused = _extract_relevant_regions(text, needles, window=40, max_chars=max_chars)
                if needle_focused:
                    return needle_focused
    return _truncate(text, max_chars)


def _read_focused_region(
    repo: Path,
    relative_path: str,
    full_text: str,
    issue_text: str,
    ...
    """Find issue-relevant line regions in a large file using rg.

    Uses rg to find lines containing issue symbols extracted from issue_text.
    Returns N=context_lines lines around each hit, with omission markers for
    gaps. Returns "" when no hits found (caller falls back to head truncation).
    """
    ...
    for symbol in symbols[:6]:
        try:
            proc = subprocess.run(
                ["rg", "--line-number", "--case-insensitive", "--fixed-strings",
                 "-e", symbol, relative_path],
                cwd=str(repo),
                stdout=subprocess.PIPE,
    ...
    for hit in hit_set:
        ...
        end = min(len(all_lines) - 1, hit + context_lines)
        for i in range(start, end + 1):
            region_set.add(i)
    sorted_idxs = sorted(region_set)
    out_lines: list = []
...
# -----------------------------
# Task-type classification (v39)
# -----------------------------

def classify_task_type(instruction: str) -> str:
    """Classify issue task type for targeted strategy injection.
    Priority order: BUGFIX > API/ROUTE > REFACTOR > UPDATE > FEATURE > OTHER
    """
    text = (instruction or "").lower()[:600]
    # Use word-boundary checks for short keywords to avoid false positives ("prefix", "suffix", "specification")
    import re as _re
    if any(_re.search(r'\b' + kw + r'\b', text) for kw in ["fix", "bug", "broken", "crash", "error", "exception", "regression", "not work", "incorrect", "wrong"]) or " fail" in text:
        # Don't classify as BUGFIX if it's clearly about adding tests to new features
        if not (any(kw in text for kw in ["implement", "create", "introduce", "new feature"]) and any(kw in text for kw in ["test", "spec"])):
            return "BUGFIX"
    if any(kw in text for kw in ["api", "route", "endpoint", "controller", "handler", "middleware", "webhook", "socket", "graphql", "rest api", "restful", "integration"]):
        return "API/ROUTE"
    if any(kw in text for kw in ["refactor", "restructure", "reorganize", "clean up", "cleanup", "simplify", "extract method"]):
        return "REFACTOR"
    if any(kw in text for kw in ["update", "change", "modify", "rename", "replace", "migrate", "upgrade", "bump", "deprecate", "remove", "delete"]):
        return "UPDATE"
    if any(kw in text for kw in ["implement", "add", "create", "introduce", "build", "support", "new feature", "integrate"]):
        return "FEATURE"
    return "OTHER"


_TASK_TYPE_PREAMBLES = {
    "BUGFIX": (
        "[Hint – bug-fix task: find the ROOT CAUSE, not just the symptom. "
        "Trace the error to its origin. The fix is usually smaller than it appears.]\n\n"
    ),
    "UPDATE": (
        "[Hint – update/change task: check ALL call-sites of the target symbol, not just the definition. "
        "Missing a call-site is the #1 failure mode for update tasks — "
        "but run rg only if the issue implies multiple locations.]\n\n"
    ),
    "REFACTOR": (
        "[Hint – refactor task: enumerate every file that imports or uses the target symbol. "
        "Update ALL of them consistently — partial refactors are penalized.]\n\n"
    ),
    "FEATURE": (
        "[Hint \u2013 new-feature task: implement ALL described requirements]\n"
        "Before editing: enumerate every requirement (main task + bullets + acceptance criteria).\n"
        "Implement them ALL \u2014 the judge checks each one. 'Partial implementation' = heavy penalty.\n"
        "Add companion tests if the repo has a test directory.\n\n"
    ),
    "API/ROUTE": (
        "[Hint \u2013 api/route task: multi-concern implementation required]\n"
        "Before any edit, audit ALL 4 integration layers:\n"
        "  Layer 1 \u2013 Route registration: is the endpoint/route defined and registered?\n"
        "  Layer 2 \u2013 Handler logic: is the controller/handler function implemented completely?\n"
        "  Layer 3 \u2013 Middleware/auth: is auth, validation, or middleware wired correctly?\n"
        "  Layer 4 \u2013 Companion tests: is the test file updated to cover the new endpoint?\n"
        "Missing any layer is heavily penalized. Check all 4 before writing code.\n\n"
    ),
    "OTHER": "",
}


_CASCADE_REQUIRED_SIGNALS = [
    "everywhere", "across", "rename ", "replace all", "move to",
    "throughout", "all files", "all instances", "all uses",
    "wherever", "all occurrences", "all references", "all callers",
    # Removed bare "all ", "any ", "each " — too many false positives on simple issues
]


def _detect_cascade_required(issue: str) -> bool:
    """True when the issue implies multi-site cascade changes (all usages must be updated)."""
    text = (issue or "").lower()[:800]
    return any(sig in text for sig in _CASCADE_REQUIRED_SIGNALS)
...
====================================================================
PATCH FORMAT EXAMPLES (format reference only — never copy content)
====================================================================

Example A — minimal single-file fix:
```diff
...
Example B — source + companion test (two files):
```diff
...
@@ -15,0 +15,4 @@
+def test_empty_body():
+    r = client.post('/api', data=None)
+    assert r.status_code == 400
```

Example C — config + source cascade:
```diff
...
def build_initial_user_prompt(issue: str, repo_summary: str, preloaded_context: str = "") -> str:
    # Inject task-type strategy preamble (v39)
    task_type = classify_task_type(issue)
    type_preamble = _TASK_TYPE_PREAMBLES.get(task_type, "")

    # Cascade hint for multi-site change detection (v39)
    if _detect_cascade_required(issue):
        cascade_hint = (
            "[Hint – multi-site change detected: run `rg -n 'SYMBOL' .` to find ALL usages before editing. "
            "Edit every file that contains a reference — do not stop at the definition.]\n\n"
        )
    else:
        cascade_hint = ""

    suffix_hint = type_preamble + cascade_hint
...
{repo_summary}
{context_section}
{suffix_hint}Before planning, read the ENTIRE issue above and identify every requirement (there may be more than one). Your patch must satisfy ALL of them — the LLM judge penalizes incomplete solutions.
...
Before making any edits, identify the most relevant existing test file for the module you will change. You will need it for verification in self-check.
...
def _self_check_type_cue(issue_text: str) -> str:
    ...
    # --- v40 MOAT: Python-specific type cues (king doesn't have these) ---
    if any(tok in text for tok in ("decorator", "@")):
        return (
            "TYPE-AWARE CUE: this issue involves decorators. Verify all call sites "
            "and stacked decorators are updated \u2014 decorator order matters and missing "
            "a wrapper breaks the decorated function silently.\n\n"
        )
    if any(tok in text for tok in ("async", "await", "coroutine", "asyncio")):
        return (
            "TYPE-AWARE CUE: this issue involves async code. Verify async/await "
            "consistency throughout the call chain \u2014 a missing await or sync call to "
            "an async function produces silent bugs or runtime errors.\n\n"
        )
    if any(tok in text for tok in ("generator", "yield", "__next__", "send(")):
        return (
            "TYPE-AWARE CUE: this issue involves generators. Verify the generator "
            "protocol is preserved \u2014 changing yield to return or vice versa changes "
            "the function's type and breaks all callers.\n\n"
        )
    return ""
...
def build_self_check_prompt(
    ...
    Two-pass design (v40 moat): SCOPE AUDIT first (per-file justification from v39),
    then adversarial self-check (ported from king PR#1576). Two passes > one pass.
    """
    truncated = (
        patch
        if len(patch) <= 4000
    ...
    # --- PASS 1: Per-file SCOPE AUDIT (v39 moat — king removed this, we keep it) ---
    changed_files = _extract_patch_files(patch)
    if changed_files:
        def _is_test_or_companion(fp: str) -> bool:
            pl = fp.lower()
            return (
                "/test" in pl or "test/" in pl or "spec/" in pl
                or pl.startswith("tests/") or ".test." in pl or "_test." in pl
                or "test_" in pl[:15] or ".spec." in pl or "_spec." in pl
            )

        non_test_files = [f for f in changed_files if not _is_test_or_companion(f)]
        test_files = [f for f in changed_files if _is_test_or_companion(f)]

        if non_test_files:
            scope_str = '\n'.join(f'  - {f}' for f in non_test_files)
            test_note = (
                "\nTest files (ALWAYS keep \u2014 do NOT revert):\n"
                + '\n'.join(f'  - {f}' for f in test_files)
                + "\n"
            ) if test_files else ""
            audit_block = (
                "## SCOPE AUDIT (do this FIRST)\n"
                "Non-test files you modified:\n"
                f"{scope_str}\n{test_note}\n"
                "For each NON-TEST file, write one sentence: \""
                "I modified [filename] because the issue requires...\"\n"
                "If you CANNOT justify a non-test file "
                "\u2192 run: git checkout -- [filename]\n"
                "Do NOT revert test files \u2014 keep them if your source change requires them.\n\n"
            )
        else:
            audit_block = ""
    else:
        audit_block = ""
    # --- PASS 2: Adversarial self-check (ported from king PR#1576) ---
...
def build_requirement_verification_prompt(issue: str, patch_text: str) -> str:
    """Closed-loop requirement verification — ensures ALL issue requirements are addressed.

    This is different from adversarial self-check (which is open-ended 'find the weakest link').
    This systematically enumerates every requirement and refuses to finalize until each is checked.
    Targets the #1 loss cause: 45.5% of judge losses = 'incomplete features'.
    """
    return (
        "## REQUIREMENT VERIFICATION (do this before <final>)\n\n"
        "Extract every requirement from the issue into a numbered list:\n"
        "- Main task (the primary thing asked for)\n"
        "- Each bullet point and sub-point\n"
        "- Every clause with 'also', 'and', 'ensure', 'must', 'should', 'when', 'unless', 'only'\n"
        "- Acceptance criteria (if listed)\n"
        "- Edge cases explicitly mentioned\n\n"
        "For each requirement, write: [REQ-N] description \u2192 \u2705 DONE / \u26a0\ufe0f PARTIAL / \u274c MISSING\n\n"
        "Rules:\n"
        "- If ANY requirement is \u26a0\ufe0f PARTIAL or \u274c MISSING \u2192 emit `<command>` blocks to fix it NOW\n"
        "- After fixing, re-check that requirement\n"
        "- Only write `<final>` when ALL requirements show \u2705 DONE\n"
        "- If a requirement cannot be addressed (no relevant code found), write [REQ-N] \u2192 \u2705 N/A (explain why)\n\n"
        "Start with: 'Requirements extracted from issue:'\n"
    )
...
_PRESENTATION_DIR_MARKERS = ('/pages/', '/views/', '/components/', '/screens/', '/layouts/')


def _is_presentation_file(path: str) -> bool:
    """Detects pages/, views/, components/, screens/, layouts/ in path."""
    return any(marker in path.lower() for marker in _PRESENTATION_DIR_MARKERS)


def _patch_orphan_presentation_files(patch: str, tracked: set) -> List[str]:
    """New pages/views/components created but not referenced in router/nav.

    Finds 'new file mode' entries in the patch that live in presentation
    directories, then checks if the patch also touches any router, sidebar,
    or navigation file. Returns list of orphaned new presentation files.
    """
    new_files: List[str] = []
    router_touched = False
    nav_keywords = ('router', 'routes', 'sidebar', 'navbar', 'navigation', 'nav.', 'menu',
                    'app.tsx', 'app.jsx', 'app.js', 'app.vue', 'index.tsx', 'index.jsx',
                    '_app.', 'layout.')
    for line in patch.splitlines():
        if line.startswith('diff --git a/'):
            parts = line.split(' b/')
            if len(parts) >= 2:
                ...
                if any(kw in fpath_lower for kw in nav_keywords):
                    router_touched = True
        if line.startswith('+++ b/'):
            ...
            if any(kw in fpath_lower for kw in nav_keywords):
                router_touched = True
        if line.startswith('new file mode'):
            # The file path was in the preceding diff --git line
            pass
    # Re-scan to find new files in presentation dirs
    lines = patch.splitlines()
    for i, line in enumerate(lines):
        if line.startswith('new file mode'):
            # Look backward for the diff --git line
            for j in range(i - 1, max(i - 3, -1), -1):
                if lines[j].startswith('diff --git a/'):
                    parts = lines[j].split(' b/')
                    if len(parts) >= 2:
                        fpath = parts[-1].strip()
                        if _is_presentation_file(fpath):
                            new_files.append(fpath)
                    break
    if router_touched or not new_files:
        return []
    return new_files


def build_wiring_nudge_prompt(orphans: List[str], issue_text: str) -> str:
    """Prompt: your patch creates new UI files but doesn't wire them into router/nav."""
    orphan_list = "\n".join(f"  - {f}" for f in orphans[:5])
    short_issue = issue_text[:1200] if len(issue_text) > 1200 else issue_text
    return (
        "Wiring gap \u2014 your patch creates new presentation files but doesn't "
        "wire them into the router, sidebar, or navigation:\n"
        f"{orphan_list}\n\n"
        "For each new page/view/component above:\n"
        "  1. Find the router or route config file and add the route\n"
        "  2. Find the navbar/sidebar/menu and add the nav link\n"
        "  3. Add any missing imports\n\n"
        "Do this now, then verify and emit <final>summary</final>.\n\n"
        "Task:\n"
        f"{short_issue}\n"
    )
...
def build_attempt2_bootstrap(result1: Dict[str, Any], n_lines: int) -> str:
    ...
    prior_patch = result1.get("patch", "") or result1.get("diff", "") or ""
    ...
    if prior_files:
        changed_str = ", ".join(prior_files[:5])
...
def build_attempt2_thin_patch_bootstrap(result1: Dict[str, Any], issue: str) -> str:
    """Bootstrap for attempt 2 when attempt 1 produced suspiciously thin patch.

    Fires when attempt 1 produced ≥3 substantive lines but fewer than
    _THIN_PATCH_LINE_THRESHOLD — technically non-empty but suspiciously
    brief for a task that likely has multiple requirements. Tells attempt 2
    to check coverage of ALL issue requirements before writing code.
    """
    patch_text = result1.get("patch", "")
    files_touched = _extract_patch_files(patch_text)
    files_str = ', '.join(files_touched) if files_touched else 'unknown files'
    return (
        f"ATTEMPT 2 \u2014 THIN PATCH DETECTED\n\n"
        f"Your first attempt produced fewer than {_THIN_PATCH_LINE_THRESHOLD} substantive lines.\n"
        f"Files touched in attempt 1: {files_str}\n\n"
        f"The issue likely requires a more complete implementation. In this attempt:\n"
        f"1. Re-read the issue requirements carefully\n"
        f"2. Identify ALL code paths and entry points that need changes\n"
        f"3. Implement the complete solution, not just a minimal stub\n"
        f"Focus on what's MISSING from attempt 1, not on repeating what's already there."
    )
...
# Thin-patch completeness trigger: if attempt 1 passes the low-signal gate
# (≥3 substantive lines) but produces suspiciously few lines for a complex
# multi-bullet issue, fire attempt 2 with a completeness-focused bootstrap.
# This catches "produces a patch but misses half the requirements" failure mode,
# distinct from under-delivery (too few files) and low-signal (< 3 lines).
_THIN_PATCH_LINE_THRESHOLD = 20


def _count_req_signals(patch: str, issue: str) -> int:
    """Heuristic: count how many issue keyword tokens appear in the patch lines.

    Used for quality-based multishot selection — picks the patch that covers
    more of the issue's requirements rather than just the bigger patch.
    """
    issue_tokens = set(
        w.lower()
        for w in re.findall(r'[a-zA-Z_][a-zA-Z0-9_]{3,}', issue)
        if len(w) > 4
    )
    patch_tokens = set(
        w.lower()
        for w in re.findall(r'[a-zA-Z_][a-zA-Z0-9_]{3,}', patch)
    )
    return len(issue_tokens & patch_tokens)
...
def _solve_with_safety_net(**kwargs: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # Thin-patch completeness trigger: passed low-signal gate but suspiciously
        # few lines for a complex multi-requirement issue. Distinct from under-delivery
        # (which checks file count). Catches "patch exists but is too shallow" failure.
        # Only fires for genuinely complex issues (>50 words or ≥2 bullet points) —
        # avoids wasting budget on simple issues where a thin patch is legitimate.
        _issue_word_count = len(_issue_text.split()) if _issue_text else 0
        _issue_bullet_count = sum(
            1 for line in (_issue_text or "").splitlines()
            if line.strip() and line.strip()[0] in ('-', '*', '•') or
            (len(line.strip()) > 1 and line.strip()[0].isdigit() and line.strip()[1] in ('.', ')'))
        )
        _issue_complex = _issue_word_count > 50 or _issue_bullet_count >= 2
        _thin_patch = (
            _n1 >= _MULTISHOT_LOW_SIGNAL_THRESHOLD   # passed low-signal (≥3 lines)
            and _n1 < _THIN_PATCH_LINE_THRESHOLD      # but < 20 substantive lines
            and not _underdelivered                    # not already caught by file-count check
            and _elapsed < _UNDERDELIVER_MAX_FIRST_ELAPSED  # attempt 1 didn't use all time
            and _issue_complex                         # only for genuinely complex issues
        )

        if _n1 >= _MULTISHOT_LOW_SIGNAL_THRESHOLD and not _underdelivered and not _thin_patch:
        ...
        if _underdelivered:
            ...
            # Thin-patch: focused completeness bootstrap, same budget bounds
            _attempt2_budget = max(
                _ATTEMPT2_BUDGET_MIN,
                min(_ATTEMPT2_BUDGET_MAX, _remaining - _MULTISHOT_MIN_ATTEMPT_RESERVE),
            )
            _bootstrap = build_attempt2_thin_patch_bootstrap(_result1, _issue_text)
        else:
        ...
        # v41: quality-based multishot selection using requirement-coverage heuristic.
        _req_score_1 = _count_req_signals(_patch1 or "", _issue_text or "")
        _req_score_2 = _count_req_signals(_patch2 or "", _issue_text or "")
        _attempt2_wins = (
            (_underdelivered and _files2 > _actual_files_1 and _n2 >= _MULTISHOT_LOW_SIGNAL_THRESHOLD)
            or (_underdelivered and _req_score_2 > _req_score_1 and _n2 >= _MULTISHOT_LOW_SIGNAL_THRESHOLD)
            or (_thin_patch and _n2 > _n1)
            or (not _underdelivered and not _thin_patch and (
                _n2 > _n1 * 1.2
                or (_req_score_2 > _req_score_1 * 1.3 and _n2 > _n1 * 0.8)
            ))
        )
        if _attempt2_wins:
...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    def maybe_queue_refinement(assistant_text: str) -> bool:
        ...
        """
        nonlocal polish_turns_used, self_check_turns_used, syntax_fix_turns_used, test_fix_turns_used, coverage_nudges_used, criteria_nudges_used, hail_mary_turns_used, total_refinement_turns_used, must_edit_after_gap, must_edit_patch, gap_edit_nudges_used, deletion_nudges_used, req_verify_used, wiring_nudges_used
        ...
        # v49: Wiring gap gate — detect orphan presentation files not wired
        # into router/nav. Fires once after coverage-nudge, before polish.
        if wiring_nudges_used < MAX_WIRING_NUDGES:
            orphans = _patch_orphan_presentation_files(patch, _tracked_set_for_checks)
            if orphans:
                wiring_nudges_used += 1
                total_refinement_turns_used += 1
                must_edit_after_gap = True
                must_edit_patch = patch
                orphan_names = ", ".join(orphans[:5])
                queue_refinement_turn(
                    assistant_text,
                    build_wiring_nudge_prompt(orphans, issue),
                    f"WIRING_NUDGE_QUEUED:\n  orphans: {orphan_names}",
                )
                return True
        ...
        # v41: Closed-loop requirement verification gate.
        # Fires ONCE after self-check completes, before allowing <final>.
        # Only when patch is non-empty, has enough substance, and budget remains.
        if req_verify_used < MAX_REQ_VERIFY_TURNS:
            _n_substantive = _multishot_count_substantive(patch)
            if _n_substantive >= _REQ_VERIFY_MIN_PATCH_LINES and time_remaining() >= 40.0:
                req_verify_used += 1
                queue_refinement_turn(
                    assistant_text,
                    build_requirement_verification_prompt(issue, patch),
                    "REQ_VERIFY_QUEUED: closed-loop requirement verification",
                )
                return True
    ...
    try:
        ...
        for step in range(1, max_steps + 1):
            ...
            # --- Soft nudge: fires BEFORE hail mary (v40, ported from king PR#1576) ---
            if (
        ...
        if patch.strip() and not success:
            ...
            success = True
        # Apply scope filter: remove unrelated whole-file diffs
        if patch.strip() and issue:
            _filtered = _filter_out_of_scope_files(patch, issue, list(preloaded_files))
            if _filtered != patch:
                _removed = [
                    _m.group(1)
                    for _m in re.finditer(r'^diff --git a/.+? b/(.+?)$', patch, re.MULTILINE)
                    if _m.group(1) not in [
                        _m2.group(1)
                        for _m2 in re.finditer(r'^diff --git a/.+? b/(.+?)$', _filtered, re.MULTILINE)
                    ]
                ]
                logs.append(
                    "SCOPE_FILTER_APPLIED: removed "
                    + str(len(_removed))
                    + " out-of-scope file(s): "
                    + str(_removed[:5])
                )
