#!/usr/bin/env python3
"""Next45 -- repo-aware prompt context + filename-fired hints + read-loop
detector. Base is Next42 (2171 lines, best-ever 21W-8L/72% seed-42/30). stdlib
only; zero new imports. EXACTLY 3 changes vs Next42.

================================= NEXT45 (this version) =====================
GATE-44 (Next42 + 2 tweaks) 10-task seed-42 = 7W-3L (70%) FAIL. Win-by-type:
API/ROUTE 3/3 (100%), FEATURE 1/1 (100%), BUGFIX 3/6 (50%) -- BUGFIX is the
structural weakness. The three BUGFIX losses and their CONFIRMED root causes
(from direct source read of solve()/run_agent_loop()/build_initial_user_prompt):

  * T8 TS DI collapse  us 0.000 vs king 0.720 (src/di/Container.ts, 5f).
    CONFIRMED BUG: the harness passes solve() ONLY `repo_path` + `issue` (the
    TITLE text). solve() then calls build_initial_user_prompt(issue, "", "") with
    an EMPTY repo_summary -- the agent gets NO file list up front. T8's title
    ("Improve Streamable HTTP Server Error Handling and Element Reload Logic")
    contains NO "Container"/"DI"/".ts", so BOTH `_CONTAINER_DI_RE` and
    `_STATIC_LANG_RE` were silent -- the word "Container" lives only in the
    FILENAME `src/di/Container.ts`. The agent flew blind on a hard TS DI task -> 0.000.
  * T3 TS BUGFIX 9-file  us 0.320 vs king 0.580. Same root: no language in title
    -> `_STATIC_LANG_RE` precision hint silent; 9-file TS scope -> read-spin /
    over-implementation.
  * T11/T4 large-Python (14f/10f): step-burn -- run_agent_loop has ZERO
    read-loop detection (king-verbatim), so the model re-reads the same regions
    and runs out of its 50-step budget before a complete patch.

THE EXACTLY THREE CHANGES vs Next42 (stdlib only, locked surfaces untouched):
  * NEXT45 CHANGE 1 (CRITICAL -- fixes T8 0.000 + cuts T3/T4/T11 step-burn):
    build a REAL repo file-list inside solve() (cheap `git ls-files`, capped) and
    pass it as `repo_summary` to build_initial_user_prompt(). This (a) puts the
    repository file tree into the FIRST prompt so the model navigates instead of
    blindly cat-ing files, and (b) gives the filename-based detectors actual
    filenames to fire on. The brief assumed the harness passes filenames via
    repo_summary -- it does NOT (always ""); this change creates that context.
  * NEXT45 CHANGE 2 (fixes T8 DI + T3 static-lang silence): the DI/Container and
    static-language detectors in build_initial_user_prompt() now search
    `issue + repo_summary` (the CHANGE-1 file list) instead of `issue` alone, so
    `src/di/Container.ts` fires the DI read-Container hint and `.ts`/`.go`
    filenames fire the precision "read the full owning implementation" hint even
    when the TITLE names neither. `_CONTAINER_DI_RE` is widened with DI vocabulary
    available in PATHS: `/di/`, `Injector`, `ServiceLocator`, `injectable`.
  * NEXT45 CHANGE 3 (DeerFlow bucketed read-loop detector -- fixes T3/T4/T11
    step-burn): in run_agent_loop(), track a bucketed key of each read-style
    command (cat/nl/head/tail/less/grep of a path; sed -n line-ranges bucketed by
    200, DeerFlow L-DEERFLOW-LOOPKEY-1). On the 3rd identical-region read, fold
    ONE neutral "stop reading, edit now" nudge into the SAME next observation
    (DeerFlow L-DEERFLOW-WARN-TIMING-1 -- never a standalone message). Pure
    stdlib bookkeeping; ZERO model tokens until a genuine spin fires; never
    blocks a command; resets per-run (no cross-task leakage).

LOCKED / UNCHANGED (do NOT regress): king's 6 content-based `_integration_hints`,
SYSTEM_PROMPT rider, TASK_TEMPLATE, the verify-repair gate, the anti-collapse
recovery, `render_observation`, `_CONTAINER_DI_RE` firing inside
_integration_hints (kept), `_LARGE_REPO_RE`. `_PYTHON_COMPLETE_RE` is NOT
re-introduced (it caused 8 regressions). stdlib only; zero new imports.

============================ NEXT42 (prior base) ===========================
Next42 = Next41 (king-pure) + narrow `_CONTAINER_DI_RE` DI hint restored
(proven 0.750 in G36) + general `_LARGE_REPO_RE` large-file focus hint. Best
ever: 21W-8L (72%) seed-42/30. This is the Next45 base.
...
# NEXT45 CHANGE 2: widened to also catch DI vocabulary that appears in FILE
...
# build_initial_user_prompt(), this fires the DI read-Container hint on T8
# (src/di/Container.ts) whose TITLE names no DI vocabulary.
...
# NEXT45 CHANGE 3 (DeerFlow bucketed read-loop detector, L-DEERFLOW-LOOPKEY-1):
# fixes T3/T4/T11 step-burn. These module-level helpers detect when the model
...
# real edit never counts as a read.
...
def run_agent_loop(*, config: AgentRunConfig, task: str) -> AgentOutcome:
    ...
    format_retries = 0
    # NEXT45 CHANGE 3: per-run read-spin counters (local dict -> no cross-task
    ...
    for step in range(1, max(1, config.max_steps) + 1):
        ...
        # NEXT45 CHANGE 3: count read-region repeats BEFORE executing. The nudge
        ...
        ) + loop_nudge  # NEXT45 CHANGE 3: fold read-spin nudge into SAME message
...
# NEXT45 CHANGE 1 (CRITICAL): the harness passes solve() ONLY repo_path + the
# issue TITLE -- repo_summary is ALWAYS "". The agent therefore starts with no
# file list and (a) burns steps navigating multi-file repos and (b) cannot fire
# filename-based hints (T8 DI "Container" lives only in src/di/Container.ts). This
# helper builds a compact, capped file listing from the repo ONCE before the
# prompt is built, so the model sees the file tree up front and the detectors in
# build_initial_user_prompt() have real filenames to match. Pure stdlib (git via
# subprocess, identical to collect_repo_patch); never raises; capped so it can
# never blow up the prompt on a huge repo.
_REPO_SUMMARY_MAX_FILES = 200
_REPO_SUMMARY_MAX_CHARS = 4000
...
def _repo_file_summary(repo_dir: str) -> str:
    """Return a newline-separated, capped list of tracked repo files (paths only)
    for use as the prompt's <repository_summary>. Tries `git ls-files`; falls
    back to a bounded os.walk. Always returns a string (possibly empty); never
    raises."""
    try:
...
def build_initial_user_prompt(issue: str, repo_summary: str, preloaded_context: str = "") -> str:
    ...
    # NEXT45 CHANGE 2: the filename-based detectors below must see the REPO FILE
    # LIST (passed by CHANGE 1 via repo_summary), not just the issue title. We
    # search this combined string so DI/static-language hints fire on filenames
    # (e.g. src/di/Container.ts, *.ts, *.go) even when the title names neither.
    ...
    # NEXT45 CHANGE 2: gate on `detect_text` (issue + repo file list) and
    # `_STATIC_LANG_RE` now matches .ts/.go/.rs/etc. extensions present in the
    # file list -- so T3 (TS 9f) and T8 (TS DI 5f) receive the precision
    # "read the full owning implementation" hint even though their TITLES name
    # no language. Verb gates still read `issue` (the actual task instruction).
    if (
    ...
    # NEXT45 CHANGE 2: DI/Container read-first hint, fired off the FILE LIST.
    # T8 (src/di/Container.ts) loses 0.000 -> king 0.720 because no DI vocabulary
    # is in its title; with the repo file list available the DI detector now
    # fires and steers the agent to read the Container/DI wiring before editing.
...
def solve(
    ...
    try:
        ...
        # NEXT45 CHANGE 1 (CRITICAL): build the repo file list ONCE and feed it to
        # the prompt as repo_summary. The harness never supplies one (always ""),
        # so without this the agent starts blind on every task. This single line
        # gives the model the file tree up front (cuts multi-file step-burn:
        # T3/T4/T11) AND lets build_initial_user_prompt()'s filename detectors
        # fire (T8 DI). Computed once; reused for the recovery run below.
        ...
        if not outcome.patch.strip():
            ...
            if remaining >= 60:
                ...
                # NEXT45 CHANGE 1: feed the same repo file list into the
