#!/usr/bin/env python3
"""Next60 -- BASE = king_agent.py (86f697b7, 2555L) carried through the proven
Next57 reliability changes + Next59's read-before-edit quality lever. SYSTEM_PROMPT
/ TASK_TEMPLATE / render_observation are BYTE-IDENTICAL to the king. stdlib only;
zero new imports; no sampling params.

================================ NEXT60 (this version) =====================
GATE EVIDENCE -- Next59 was our BEST EVER: 16W-9L-1T, margin = +7, 64% decisive
WR (PASSED the >50% threshold). The nine losses break down as:
  * 3 OUR_TIMEOUT rounds (T7 JS API/ROUTE, T18 Other, + one tie T17) -- inherently
    hard / large multi-file tasks where even the king also times out; NOT a
    prompt-fixable quality gap and NOT worth a timeout-risking change.
  * 2 Python BUGFIX clean losses (T2 0.500, T10 0.380) -- a quality gap, but
    Python is already nudged elsewhere and a Python-specific lever is volatile.
  * 2 JavaScript BUGFIX CLOSE losses -- T20 (ours 0.720 vs king 0.780) and T22
    (ours 0.820 vs king 0.880), both lost by only ~0.06, both with OUR cursor-sim
    telemetry ABOVE the king. These are the highest-value, lowest-risk targets.
  * 2 misc (T15 TS UPDATE 0.500 close, T23 Other empty patch).

THE EXACTLY ONE TARGETED CHANGE vs Next59 (NEXT60 CHANGE 7, prompt-only, zero
step cost -> no added timeout risk, which is our worst failure mode):
  Extend the king-proven read-before-edit nudge (already validated on TS/Go/Rust/
  Java/C++ BUGFIX tasks via _STATIC_LANG_RE) to ALSO fire on JavaScript BUGFIX
  tasks. JavaScript is the only high-volume language (7 tasks/gate) the static-
  language read-before-edit branches deliberately exclude (.js/.jsx are not in
  _STATIC_LANG_RE), so JS BUGFIX rounds get NO read-before-edit discipline today.
  T20/T22 are exactly that case: close JS BUGFIX losses where reading the FULL
  owning implementation before editing is the cheapest defensible lever toward
  the ~0.06 the king edged us by. A new narrow `_JS_LANG_RE` (matches
  .js/.jsx/.mjs/.cjs/'javascript', EXCLUDES .ts/.tsx and .json) gates a third,
  mutually-exclusive read-before-edit branch in build_initial_user_prompt():
  fires ONLY for a JS task with NO construction verb (a true BUGFIX) when neither
  static-language branch already fired -- so no prompt ever stacks two read-
  before-edit blocks. Same nudge text as the static branches. No new import,
  zero step cost, byte-identical SYSTEM_PROMPT/TASK_TEMPLATE/render_observation.

NOTE: this is the quality-ceiling lever, not a timeout lever. The 3 OUR_TIMEOUT
losses (T7/T18 + tie T17) are inherently hard tasks where the king also times
out; spending a timeout-risking change to chase them would regress our best-ever
margin. We extend ONE validated quality mechanism to ONE more language and keep
everything that produced margin +7 intact.

UNCHANGED vs Next59 (do NOT regress): all five Next57 changes (270/30 budget,
per-call request_timeout bound, checkpoint-to-disk, crash guard, injection
neutralizer + Rust hint) AND Next59 CHANGE 6 (static-language read-before-edit
extension). NO global hard-exit floor. NO scope-guard-violating checkpoint
signature (repo_path: str, patch_text: str) -> None. stdlib only; zero new
imports; no sampling params.

============================ NEXT59 (prior version) ========================
Next59 -- BASE = agent_cl_gpt_Next57.py (which is king_agent.py 86f697b7,
2555L + the five Next57 reliability changes). SYSTEM_PROMPT / TASK_TEMPLATE /
render_observation are BYTE-IDENTICAL to the king. stdlib only; zero new
imports; no sampling params.

================================ NEXT59 (prior version) =====================
WHY Next59 REVERTS TO Next57 AS THE BASE (drops Next58's global hard-exit floor):
  Gate evidence on the king 86f697b7:
    * Next57: 14W-12L-2T  margin = +2   (4 timeouts)
    * Next58: 8W-9L-3T    margin = -1   (Next57 + a GLOBAL 275s hard-exit floor)
  Next58 was Next57 PLUS one change: a global hard-exit floor that breaks the
  agent loop at the TOP of every step once GLOBAL elapsed >= 275s (300 - 25s
  reserve), measured from a single solve()-entry anchor. Task-by-task log diff
  (gate_next57 vs gate_next58) shows the floor is (a) REDUNDANT with Next57's
  per-call request_timeout bound (CHANGE 2) -- which already guarantees no single
  model call can overrun the 300s wall and that solve() returns the on-disk diff
  before SIGKILL -- and (b) net-harmful: at 275-295s the agent is often still
  finishing an edit or its final verify/submit step, and the floor aborts that
  work early, returning a staler partial. The dominant win-variance between the
  two runs is KING NON-DETERMINISM (the king timed out on ~8 rounds in the
  Next57 run but far fewer in the Next58 run, flipping several wins to losses
  independent of our code) -- but our OWN raw scores also dropped on tasks where
  Next58 cut the loop early (T1 0.550->0.480, T3 0.380->0.280). Reverting the
  floor recovers Next57's margin=+2 baseline. The per-call timeout bound
  (CHANGE 2) is the correct, sufficient SIGKILL guard; the global floor adds
  aggression without a corresponding reliability gain.

THE EXACTLY ONE TARGETED CHANGE vs Next57 (NEXT59 CHANGE 6, prompt-only, zero
step cost -> no added timeout risk, which is our worst failure mode):
  Broaden the king-proven static-language precision hint in
  build_initial_user_prompt() so it ALSO fires on bugfix-style statically-typed
  tasks that carry NEITHER a construction verb NOR a precision-fix verb. The
  persistent loss pattern across Next57 AND Next58 is C/C++ BUGFIX (0.080-0.480),
  TypeScript BUGFIX (0.200-0.300), and mid-range Python BUGFIX -- exactly the
  tasks where uid250 reaches 0.88-1.0 by reading the FULL owning implementation
  before editing. The existing hint (_PRECISION_FIX_RE & not _CONSTRUCT_VERB_RE &
  _STATIC_LANG_RE) silently does NOT fire on many of these because their issue
  text lacks an improve/error-handling/exception verb (e.g. "Fix: Task 'mark as
  done' ..."). Next59 adds a SECOND, mutually-exclusive branch: when the task is
  statically-typed (_STATIC_LANG_RE) AND has no construction verb
  (not _CONSTRUCT_VERB_RE) AND the precision branch did NOT already fire, append
  the SAME read-the-full-implementation nudge. No new regex, no new import,
  zero step cost. This extends a gate-validated king mechanism to the documented
  loss set without touching SYSTEM_PROMPT/TASK_TEMPLATE byte-identity.

NOTE: this is the quality-ceiling lever, not a timeout lever. uid250's 1.0/0.9/
0.88 scores are WRITTEN BETTER CODE within 300s, not timeout luck; the read-
before-edit discipline is the cheapest defensible step toward closing that gap.

UNCHANGED vs Next57 (do NOT regress): all five Next57 changes (270/30 budget,
per-call request_timeout bound, checkpoint-to-disk, crash guard, injection
neutralizer + Rust hint). NO global hard-exit floor (Next58's regression --
removed). NO scope-guard-violating checkpoint signature
(repo_path: str, patch_text: str) -> None. stdlib only; zero new imports.

============================ NEXT57 (prior version) ========================
...
# NEXT60 CHANGE 7: JavaScript marker regex (for the JS-BUGFIX read-before-edit
# extension). Matches ONLY unambiguous JS markers (.js / .jsx / .mjs / .cjs file
# extensions or the word "javascript"). Deliberately EXCLUDES TypeScript (.ts /
# .tsx already covered by _STATIC_LANG_RE) so the two branches never stack, and
# excludes .json (the `\b` after js/jsx stops `.json` from matching `.js`).
_JS_LANG_RE = re.compile(
    r"\b(javascript|\.jsx?\b|\.mjs\b|\.cjs\b)",
    re.I,
)
...
def run_agent_loop(*, config: AgentRunConfig, task: str) -> AgentOutcome:
    ...
    for step in range(1, max(1, config.max_steps) + 1):
        ...
        # checkpoint via on-disk repo diff (collect_repo_patch) handled by crash guard
...
def build_initial_user_prompt(issue: str, repo_summary: str, preloaded_context: str = "") -> str:
    ...
    if (
        ...
        precision_static_fired = True
    # NEXT59 CHANGE 6 (prompt-only, zero step cost -- the ONE targeted quality
    # lever): broaden the king-proven read-before-edit hint to ALSO cover
    # bugfix-style statically-typed tasks that carry NEITHER a construction verb
    # NOR a precision-fix verb. The persistent loss set across Next57 AND Next58
    # (C/C++ BUGFIX 0.080-0.480, TypeScript BUGFIX 0.200-0.300, mid-range Python
    # BUGFIX) includes many tasks whose issue text lacks an improve/error-
    # handling/exception verb (e.g. "Fix: Task 'mark as done' ..."), so the
    # precision branch above silently never fires. uid250 reaches 0.88-1.0 on
    # exactly these tasks by reading the FULL owning implementation before
    # editing. This second, mutually-exclusive branch appends the SAME nudge for
    # any statically-typed (_STATIC_LANG_RE) non-construction task when the
    # precision branch did not already fire. No new regex, no new import, zero
    # step cost; extends a gate-validated king mechanism to the documented loss
    # set without touching SYSTEM_PROMPT/TASK_TEMPLATE byte-identity.
    elif (
        not precision_static_fired
        and not _CONSTRUCT_VERB_RE.search(issue)
        and _STATIC_LANG_RE.search(issue)
    ):
        prompt = prompt + (
    ...
    # NEXT60 CHANGE 7 (prompt-only, zero step cost -- the ONE targeted quality
    # lever vs Next59): extend the SAME king-proven read-before-edit nudge to
    # JavaScript BUGFIX tasks. Next59's gate (16W-9L-1T, margin +7) lost T20
    # (ours 0.720 vs king 0.780) and T22 (ours 0.820 vs king 0.880) -- BOTH
    # JavaScript BUGFIX rounds we lost by only ~0.06. JavaScript is the only
    # high-volume language (7 tasks/gate) NOT covered by the static-language
    # read-before-edit branches above (_STATIC_LANG_RE intentionally excludes
    # .js/.jsx to keep TS/Go/Rust/Java/C++ scoping clean). On those two close JS
    # losses we already out-scored the king on cursor-sim telemetry; the ~0.06
    # judge gap is a quality edge the read-the-full-implementation discipline
    # directly targets. This third, mutually-exclusive branch fires ONLY for a
    # JavaScript task (_JS_LANG_RE) that is a true BUGFIX (no construction verb)
    # and where neither static-language branch already fired -- so no prompt
    # ever stacks two read-before-edit blocks. No new import, zero step cost, no
    # SYSTEM_PROMPT/TASK_TEMPLATE change. Identical nudge text as the static
    # branches (a single validated lever extended to one more language).
    elif (
        not precision_static_fired
        and not _CONSTRUCT_VERB_RE.search(issue)
        and _JS_LANG_RE.search(issue)
    ):
        prompt = prompt + (
            "\n[Read the FULL implementation of the affected class/module before "
            "making any edit -- do not patch callers without understanding the "
...
# _t68_ckpt_write / _t68_ckpt_read removed — validator scope guard owns checkpoint
# function signatures. Crash guard falls back to collect_repo_patch() (on-disk diff).
