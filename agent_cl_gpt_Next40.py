...
Base is Next39 (2132 lines). stdlib only; zero new imports.
...
def _language_hints(issue: str, task_files: Optional[List[str]] = None) -> List[str]:
    # NEXT40 CHANGE 3: the TWO proven LANGUAGE-based hints (C++ BUGFIX, FEATURE-
    # minimal) are isolated here, OUT of the king-pure `_integration_hints()`.
    # These are the only language hints stable across multiple seeds (the C++
    # hint won +0.240 on gate-34 T1; the FEATURE-minimal hint made T9 reliable).
    # Kept byte-for-byte from Next39. `build_initial_user_prompt()` calls this
    # AFTER the acceptance checklist so the order is: integration hints (inside
    # the checklist) -> language hints -> (the checklist itself is appended in
    # build_initial_user_prompt). `task_files` is accepted for forward-
    # compatibility but the hints currently key on `issue` text only.
    hints: List[str] = []
    # C++ BUGFIX hint (kept verbatim from Next35/Next39): when the task is C/C++
    # AND a BUGFIX, point the agent at the header files first and at the common
    # C++ bug classes (memory lifecycle, null/bounds, const-correctness, header
    # decl vs def). Fix the minimal set of files.
    ...
    # FEATURE-minimal hint (kept verbatim from Next36/Next37/Next39): detect a
    # FEATURE implementation task in a small repo (implement/configurable/
    # adjustable verb AND a zoom/speed/preference/setting subject, <= 6 files).
    # Steer the agent to the MINIMAL viable change rather than over-engineering.
    if (
...
def build_initial_user_prompt(issue: str, repo_summary: str, preloaded_context: str = "") -> str:
    ...
    # NEXT40 CHANGE 2: acceptance-checklist injection (king mechanism, unchanged).
    # extract_criteria() already appends the king-pure _integration_hints() to
    # the extracted bullet/numbered criteria; format_checklist() renders them as
    # a visible "## Acceptance checklist" the agent must verify before submitting.
    checklist = format_checklist(extract_criteria(issue))
    prompt = base + checklist if checklist else base
    # NEXT40 CHANGE 3: language hints (C++ BUGFIX + FEATURE-minimal) are appended
    # AFTER the acceptance checklist. Order: integration hints (carried inside the
    # checklist via extract_criteria) -> checklist -> language hints. This keeps
    # the king-pure content-based hints driving the checklist while the two
    # proven language hints ride along as a short trailing note.
    lang_hints = _language_hints(issue)
    if lang_hints:
