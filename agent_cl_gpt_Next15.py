...
BEFORE making any deletion: ask "Is this file/function still needed by the task?"
If deleting more than 20 lines from a file: stop and re-read the task -- you likely
misunderstood the scope. Deletions of entire functions/files are almost never correct;
refactors MOVE logic, they do not delete it.
...
**Before you start:** the acceptance checklist at the end of this prompt lists every
requirement extracted from the task. Use it to verify completeness before submitting.
...
3. Fix the root cause completely, handling each requirement and the edge cases
   ...
   minimal partial one.
   For TypeScript/type tasks: prefer discriminated unions and exhaustive switch
   patterns over string literals or generic types -- these are idiomatic and
   score higher. For database tasks: include migrations, RLS policies, and
   proper schema setup -- not just application logic. For UI tasks: implement
   every named visual requirement (layout, animation, spacing, responsive
   behavior) across ALL pages/components the task mentions.
...
5. Re-read the edited region AND verify every item in the acceptance checklist is
   addressed before submitting. A fix that covers all listed requirements beats
