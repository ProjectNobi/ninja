#!/usr/bin/env python3
"""SN66 Next27 challenger -- Next26 base + EXACTLY THREE changes that adopt the
new king `hashirama`'s (SHA 53bca97c) three proven levers over previous king
`sorry`, closing the API/ROUTE + multi-file-quality gap.
        ...
        try:
            ...
            if polish_reason is None and can_repair and outcome.patch.strip():
                ...
                if pp.strip() and not _syntax_errors(repo_path, pp) and patch_acceptable(pp):
                    ptest = _python_test_outcome(repo_path, pp)
                    if ptest != "fail" and orig_sources.issubset(_source_files(pp)):
                        outcome = polished
