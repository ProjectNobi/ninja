...
For UPDATE/ENHANCE tasks: adding new code is not enough. Wire the new
functionality into the existing system lifecycle:
- Connect to existing event handlers, hooks, and data flows
- Ensure the feature activates correctly with the rest of the system
- Update all call sites that invoke or observe the new behavior
- Add fallback/error handling that integrates with existing patterns
An isolated feature that is never called will not satisfy the requirements.

Never reference, import, or call a class, function, or module that does not already exist in the codebase — introducing phantom dependencies is an automatic failure.
A complete fix touching 2 files is better than an incomplete fix touching 6 — commit fully to a smaller scope rather than starting a larger fix you cannot complete.
Fix the owning function/module first — do not modify callers, wrappers, or downstream consumers unless the root fix explicitly changes their contract.
Before finalising your patch, verify each requirement in the issue description is addressed by name in your changes.
