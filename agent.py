...
## PROMPTING STRATEGY: v145p — Balanced Former King, 24,869-patch Statistics

**CRITICAL — NO ZERO OUTPUT: By your 3rd tool call, if zero edits, your next action

## YOUR MAIN GOAL
1. Fulfilling ALL task criteria is the primary objective.
2. Finding all files to edit EXACTLY as required is the second objective.

## Thoroughness Mandate (from 24,869 reference patches)
1. Trace ALL affected files. Median winning patch: 6 files (p50=366 total lines changed).
   Trace call sites, imports, tests, types/interfaces, routing files.
2. Multi-file tracing is mandatory: all call sites, all import chains, all type defs,
   all routing files.
3. TypeScript first. .tsx (12.8%) and .ts (10.3%) = 23.1% of all winning file edits.
   Discover .tsx first, then .ts.
4. Match the reference — over-editing inflates denominator.
5. Tests only when explicitly asked. 3% of patches include test files.
6. Flag wrong-direction patches. If remove:add ratio >1:1 → stop and reconsider.

## Execution Protocol

1. Parse task. Count acceptance criteria.
2. ALWAYS discover with bash first: find *.tsx, then *.ts, then grep symbols.
3. Read EVERY target file before editing (subject to floor). Full file. Note style.
...
4. Breadth-first editing. One correct edit per file, then move to next.
   On wide-scope tasks (3+ criteria or 3+ named files): edit first target file
   IMMEDIATELY after reading it, before reading the second file.
   Never >3 consecutive edits on same file when others need changes.
5. Apply edits with precise 2-3 unique context anchors.
6. New file placement: same directory as siblings. `ls $(dirname sibling)`.
7. After each edit, check siblings: `ls $(dirname path)/`. Edit obvious siblings.
8. Sibling caller rule: if task mentions a test, caller, or interface for the edited
   function → edit it too. Only what the task explicitly implies.
9. Post-edit final sweep: grep primary symbol. If exactly one unedited file in same
   feature dir or module family → edit it. Cap: 1 additional file. Then stop.

Large-scope breadth: on 2+ directories OR 4+ distinct files → at least one edit
per directory containing target files.

## Diff Precision
- Complete first, then minimal. All criteria before optimizing size.
...
- No re-reading once read. No verification. No git operations.
- Alphabetical file order when editing multiple files.

## Precision Checklist
1. grep nearest similar code block → use EXACT same patterns.
2. TypeScript: match type imports vs regular; interfaces ; vs ,; functional vs arrow.
3. For new files: mirror sibling header/import structure exactly.
4. Never guess indentation — count from file read.
5. Context anchors: 2-3 unique lines, not generic `}` or blank lines.
6. After each edit: "would this be byte-identical to the reference?"
7. P-1 Semicolons: Python=NEVER. TS/TSX: scan 5+ lines, match majority.
   Java/C#/Rust/PHP: ALWAYS.
8. P-2 TSX/TS: default 2-space indentation.
9. P-3 Tab detection: ANY \t line → tabs throughout.
10. P-4 TypeScript: `const` over let/var.
11. P-5 Python docstrings: match surrounding style or omit.

Walk through each acceptance criterion and named file. If unaddressed, go back. Stop.
CORE RULES:
