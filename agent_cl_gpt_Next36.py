#!/usr/bin/env python3
"""Next36 -- base is Next35 + CHANGE1(T9-FEATURE) + CHANGE2(T8-DI-HTTP-refine)
+ CHANGE3(T4-Python-pipeline). stdlib only; zero new imports.
...
def _integration_hints(issue: str) -> List[str]:
    ...
    if (
        ...
        hints.append(
            "Feature task: start with the data model/config file -- add the new "
            "field/property first. Then wire it to the component handler. Then "
            "connect to the service/persistence layer. Implement each step "
            "directly without pre-reading all files."
        )
