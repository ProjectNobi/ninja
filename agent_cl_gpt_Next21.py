#!/usr/bin/env python3
"""SN66 Next21 challenger -- Next20 + one surgical fix: tighten _is_large_scope().
...
# 'add' and 'update' are included because they only count when condition (a)
# (API keyword) is already confirmed, making false positives impossible.
_LARGE_SCOPE_VERBS_RE = re.compile(
    r"\b(create|implement|build|introduce|develop|(?:set\s+up)|integrate"
    r"|establish|register|configure|inject|mount|instantiate|add|update|extend)\b",
    re.I,
)
...
def _is_large_scope(issue: str) -> bool:
    """True ONLY when task is a genuine API/architecture build with multiple components.
    NEXT21 CHANGE 6: requires BOTH API keywords AND 2+ construction verbs.
    Simple BUGFIXes never trigger even if they mention verbs like 'fix' or 'add'.

    FIRES:    'Enhance Repository Analysis Backend with Full AI Pipeline. Create route,
               implement auth middleware, register service.'  (API keyword + 3 verbs)
    FIRES:    'Implement Password Reset Functionality. Build POST /reset endpoint,
               create ResetToken model, integrate email service.'  (auth + 3 verbs)
    NO FIRE:  'Fix the P2P sync robustness and offline catch-up for SkyFS'  (no API keyword)
    NO FIRE:  'Improve Streamable HTTP Server Error Handling'  (no API keyword, no construction verbs)
    NO FIRE:  'Enhance Password Manager Functionality'  (no API keyword)
    NO FIRE:  'Refactor Node Styling and Introduce New Texture Options'  (no API keyword)
    """
    text = issue or ""
    # Condition (a): strong API/architecture keyword required
    if not _API_ROUTE_RE.search(text):
        return False
    # Condition (b): 2+ distinct construction verbs required
    matches = _LARGE_SCOPE_VERBS_RE.findall(text)
    # deduplicate to count distinct verbs
    distinct = {m.lower().replace(" ", "") for m in matches}
