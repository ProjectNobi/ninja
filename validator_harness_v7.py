...
def run_agent(
    ...
    try:
        ...
        # the wall still submits whatever it wrote to disk. This runner used to
        # read the patch only from solve()'s return value and so scored every
        # timeout a bare 0.00 -- penalising tasks the agent would NOT zero live.
        # Recover the on-disk diff (modified + intent-to-add new files).
        timeout_patch = ""
        try:
