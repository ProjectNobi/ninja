...
FIXES FROM v6 (all verified against validate_live_reference.py with line citations):
  ...
  2. REFERENCE PATCH — was: NOT passed to judge.
                       now: passed as "reference_patch_privileged_context"
...
JUDGE_MODEL            = "anthropic/claude-sonnet-4.6"      # :77 _DIFF_JUDGE_MODEL
JUDGE_MODEL_FALLBACK   = "moonshotai/kimi-k2.6"             # :78 _DIFF_JUDGE_FALLBACK_MODELS
JUDGE_MODELS           = (JUDGE_MODEL, JUDGE_MODEL_FALLBACK) # :89 _DIFF_JUDGE_MODELS (ordered)
...
def _build_judge_content(task_text: str, reference_patch: str,
    ...
    Reference patch is passed as 'reference_patch_privileged_context' (the dominant signal)."""
    return [
            ...
            "text": json.dumps(
                {
                    ...
                    "reference_patch_privileged_context": _truncate_middle(
                        reference_patch, JUDGE_MAX_PATCH_CHARS),
                },
...
def _build_judge_string(task_text: str, reference_patch: str,
    ...
    payload = {
        "task": _truncate_middle(task_text, JUDGE_MAX_TASK_CHARS),
        "reference_patch_privileged_context": _truncate_middle(reference_patch, JUDGE_MAX_PATCH_CHARS),
