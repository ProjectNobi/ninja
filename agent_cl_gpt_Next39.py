...
Base is Next37. stdlib only; zero new imports.
...
# NEXT39: the old _CONTAINER_DI_RE guard is gone (that regex was removed); the
# specific subject+verb gate cannot collide with a DI bugfix on its own.
...
def _integration_hints(issue: str) -> List[str]:
    ...
    # NEXT39 CHANGE 2: DI branch REMOVED (was _PRECISION_FIX_RE + _CONTAINER_DI_RE).
    # It scored 0.750 on one DI task in G36 but 0.060 on a different DI task in
    # the G37 30-task -- overfit. The upgraded TASK_TEMPLATE (read-in-full +
    # precise edits + wire-every-symbol) handles DI tasks generically.
    ...
    # NEXT39 CHANGE 2: scoped JS frontend hint REMOVED (was _JS_FRONTEND_RE +
    # _FRONTEND_ENHANCE_RE). It won 0.400 on the 10-task seed but lost 0.280 on a
    # different JS task in the 30-task gate -- overfit. JS component tasks are now
    # covered by the upgraded TASK_TEMPLATE.
    #
    # NEXT39 CHANGE 2 (simplified Go hint): fires ONLY on a large Go task (>10
    # files) that mentions sync/goroutine/channel, emitting ONE sentence. The
    # prior broad integration-test steering was unproven on the 30-task gate.
    if (
        _GO_LANG_RE.search(issue)
        and _GO_SYNC_RE.search(issue)
        and len(_FILE_EXT_RE.findall(issue)) > 10
    ):
        hints.append(
            "Large Go task: identify the primary goroutine/channel owning this "
            "feature before making any changes."
        )
    # NEXT36 CHANGE 1 (T9 FEATURE-TypeScript collapse fix, KEPT -- T9 now
    # reliable): detect a FEATURE implementation task in a small repo -- an
    # implement/configurable/adjustable verb AND a zoom/speed/preference/setting
    # subject, with file count <= 6. Steer the agent to the MINIMAL viable change
    # (add the single field, reuse existing infrastructure) rather than
    # over-engineering. NEXT39: the stale _CONTAINER_DI_RE guard is dropped (that
    # regex no longer exists); the FEATURE subject/verb gate is already specific
    # enough that it cannot collide with a DI bugfix.
    if (
        _FEATURE_VERB_RE.search(issue)
    ...
    # NEXT39 CHANGE 2: Python large-pipeline focus hint REMOVED (was
    # _PYTHON_LANG_RE + _PYTHON_PIPELINE_RE + _PYTHON_PIPELINE_EXCLUDE_RE).
    # Unstable across 4+ gates and required a growing exclusion list to suppress
    # its own misfires. The upgraded TASK_TEMPLATE handles large Python backend
    # tasks generically.
    #
    # NEXT39 CHANGE 2: TypeScript style/refactor hint REMOVED (was _TS_LANG_RE +
    # _TS_BUGFIX_RE). Only 2 gates old and never validated on the 30-task
    # distribution; _STATIC_LANG_RE already covers TypeScript precision.
    return hints
