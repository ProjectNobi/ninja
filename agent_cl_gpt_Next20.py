#!/usr/bin/env python3
"""SN66 Next20 challenger -- Next19 + two targeted API/ROUTE completeness fixes.
...
Next19 gate: W6/L4 = 60% (FAILED). Root cause: API/ROUTE tasks 33% WR.
  BUGFIX: 83% ✅ (Next19 completeness fixes work perfectly)
  API/ROUTE: 33% ⚠️ — T4 catastrophic loss (us=0.18, king=0.52, -0.34 delta)
  FEATURE: 0% ⚠️ — close loss (us=0.20, king=0.28, -0.08 delta)
  Only T4 is a bad loss. T5/T9 are close (-0.09/-0.08) and should flip.

Next20's two (and ONLY two) new changes vs Next19 (M2.7 design, T68Bot build):
  (All three Next19 changes are kept: checklist interception, completeness_check
  repair, convergence framing. These are only additions on top of Next19.)

  * CHANGE 4 (scope-planning turn for large tasks): Detect large-scope tasks by
    counting distinct "Create/Add/Implement" requirements or API-keyword presence.
    If count >= 3 OR task contains route/endpoint/pipeline/API keywords:
    Inject ONE planning turn at loop start: "list every file you will
    create/modify and what each will contain. Reply ONLY with the file list."
    Store plan in context, then proceed with normal coding loop.
    Directly targets T4-style catastrophic scope misses (built something but
    missed most of the pipeline). Costs 1 step on large tasks only.
    (M2.7 design, Change A from Next20 design session.)

  * CHANGE 5 (API auto-criteria injection): In extract_criteria(), when the task
    is detected as API/ROUTE type, automatically append 3 wiring criteria:
    - "Every endpoint described must be wired to router/app with correct HTTP method"
    - "All new modules/classes must be imported where they are used"
    - "The API must be reachable from the entry point (app.py/main.py/index.js)"
    Zero cost. Directly targets T5/T9 close losses where wiring was likely the gap.
    The Change 1 pre-submit interception and Change 2 completeness_check repair
    will now automatically fire if any of these wiring criteria are missed.
    (M2.7 design, Change B from Next20 design session.)

All Next19 changes kept:
...
# NEXT20 CHANGE 4+5: Large-scope / API-route detection.
# Used by: (4) planning turn injection, (5) API auto-criteria.
_API_ROUTE_RE = re.compile(
    r"\b(route|endpoint|API|pipeline|authentication|auth|service|controller|"
    r"middleware|handler|webhook|REST|GraphQL|schema|migration|model|repository|"
    r"database|ORM|CRUD|FastAPI|Flask|Express|Django|Laravel)\b",
    re.I,
)
_LARGE_SCOPE_VERBS_RE = re.compile(
    r"\b(create|add|implement|build|introduce|develop|write|set up|integrate|establish)\s+",
    re.I,
)
...
def _is_large_scope(issue: str) -> bool:
    """True when the task is API/ROUTE or has >= 3 distinct build requirements."""
    if _API_ROUTE_RE.search(issue or ""):
        return True
    matches = _LARGE_SCOPE_VERBS_RE.findall(issue or "")
