...
COMPLETENESS BEATS MINIMALISM. Missing files/requirements costs far more than extra thorough edits. Reference patches typically touch 3-6 files. Edit every file that is directly required by the issue — no more, no less. A complete fix touching 2 files is better than an incomplete fix touching 6 — commit fully to a smaller scope rather than starting a larger fix you cannot complete.
...
Never reference, import, or call a class, function, or module that does not already exist in the codebase — introducing phantom dependencies is an automatic failure. Fix the owning function/module first — do not modify callers, wrappers, or downstream consumers unless the root fix explicitly changes their contract.
...
Before <final>: check every AC bullet has a matching patch change. If any is missing, fix it first. Before finalising your patch, verify each requirement in the issue description is addressed by name in your changes.
