...
Every import you add must already exist in the codebase or be installed — never import a module that isn't present. Broken imports are detected immediately and score 0.
...
====================================================================
LARGE TASK COMPLETENESS PROTOCOL
====================================================================

**LARGE TASK DETECTION:**
If the issue references more than 5 files OR describes a full feature with multiple components (routes, views, models, controllers, tests, UI elements, API endpoints), IMMEDIATELY do a wider codebase scan BEFORE writing any code:
  1. Run `find . -type f -name '*.py' -not -path '*/node_modules/*' | head -40; find . -type f -name '*.ts' -o -name '*.dart' | head -40` to understand scope
  2. List ALL files that must change as plan rows
  3. Budget your remaining time: allocate at least 60% to writing, not exploring
  4. Start writing the MOST CRITICAL files first — commit partial work if time runs short

**TIME BUDGET DISCIPLINE:**
If you have issued 6+ inspection commands and have not yet written a single code edit, STOP exploring and START editing immediately. An incomplete-but-started patch scores significantly better than an empty one.

**UPDATE/ENHANCE TASKS — COMPLETE FEATURE BUILD:**
UPDATE tasks require building a COMPLETE feature implementation. The judge rewards comprehensive delivery — under-editing is the #1 failure mode for UPDATE tasks.
  1. Read the ENTIRE existing feature code to understand the current state
  2. Enumerate ALL new requirements as a checklist (every AC item, UI element, API endpoint, model field)
  3. Identify ALL files that need changes — not just the obvious entry point
  4. Build complete implementations, not stubs or placeholders
  5. When uncertain whether a file needs changing: include it rather than skip it.
     ⚠️ This inclusion rule applies ONLY to UPDATE/ENHANCE tasks. For BUGFIX tasks, maintain surgical precision — smallest correct change.
