#!/usr/bin/env python3
"""SN66 Next25 challenger -- Next24 base + three BUGFIX-collapse fixes vs king `sorry`.
...
def render_observation(*, returncode: int, output_text: str, remaining_steps: int) -> str:
    # NEXT25 CHANGE 3: three-tier escalating step pressure that delays the
    # convergence/abort framing and inserts an explicit regression-test nudge.
    #
    # Root cause (gate-10 vs king `sorry`): BUGFIX win-rate collapsed to 33%.
    # King `sorry` only fires its single pressure note at <= 3 steps; Next24
    # fired convergence framing at <= 6, which (per the BUGFIX analysis) pushed
    # the model to SUBMIT before it wrote the focused regression test that the
    # judge credits heavily as "proof of correctness". The fix preserves the
    # anti-timeout timing benefit while giving the model room to add its test:
    #   * <= 6 : mid-pressure -- "implement your fix now AND include a focused
    #            regression test proving it works" (the test nudge). This is the
    #            new tier; it does NOT tell the agent to converge/stop, it tells
    #            it to start writing the fix + test.
    #   * <= 4 : convergence framing -- "enough to finish; write your patch,
    #            address every requirement, wire new symbols, submit" (moved
    #            from <= 6 to <= 4 so it no longer pre-empts the test).
    #   * <= 3 : FINAL -- hard "submit NOW, read no more files" directive
    #            (unchanged).
    # The note is label-blind (render_observation has no task-type context); the
    # test nudge helps BUGFIX most while never harming API/ROUTE (100%) or
    # FEATURE (100%) -- a focused test only strengthens those patches too.
    if remaining_steps <= 3:
        ...
        # Convergence framing (moved from <= 6 to <= 4 in Next25). Tells the
        # agent WHAT to do, not what to stop -- keeps the timing benefit while
        # no longer pre-empting the regression test that the <= 6 nudge primes.
        remaining_note = (
        ...
        # NEXT25 CHANGE 3 (new mid-pressure tier): explicit regression-test nudge.
        # Mirrors king `sorry`'s TASK_TEMPLATE step 4 (mandatory test in patch)
        # but reinforced at the exact step budget where the agent must commit to
        # implementing. This is the single highest-leverage change for the
        # BUGFIX collapse -- it ensures the model includes the test the judge
        # credits before convergence pressure forces a premature submit.
        remaining_note = (
            f"[{remaining_steps} steps left -- implement your fix now and make "
            f"sure to include a focused regression test that FAILS on the "
            f"original bug and PASSES with your fix, inside the patch. Keep the "
            f"change tightly scoped, then continue toward submitting.]"
        )
    else:
        remaining_note = ""
...
_CONSTRUCT_VERB_RE = re.compile(
    r"\b(implement|create|build|introduce|establish|register)\b",
    re.I,
)
_API_TASK_HINT = (
