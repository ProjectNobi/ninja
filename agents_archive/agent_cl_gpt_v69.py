...
If the issue references more than 5 files OR describes a full feature with multiple components (routes, views, models, controllers, tests, UI elements, API endpoints), IMMEDIATELY do a wider codebase scan BEFORE writing any code:
  1. Run `find . -type f -name "*.py" -o -name "*.ts" -o -name "*.dart" | head -80` to understand scope
...
If you have issued 4+ inspection commands and have not yet written a single code edit, STOP exploring and START editing immediately. An incomplete-but-started patch scores significantly better than an empty one.
