#!/usr/bin/env python3
"""SN66 Next34 challenger -- C++-BUGFIX + API/ROUTE-FIRST-ACTION + GO-LARGE-REPO.
Base is Next33 + EXACTLY THREE surgical changes derived from the gate-33 results
(4 losses: T1 C++ BUGFIX, T5 PHP API/ROUTE, T6 Go BUGFIX, T7 JS API/ROUTE).
stdlib only; zero new imports.

============================= NEXT34 (this version) =========================
GATE-33 EVIDENCE (6W-4L = 60%, threshold 80% -- NOT COMPETITIVE):
  | # | Type      | Lang  | Us    | King  | Gap    |
  | T1| BUGFIX    | C/C++ | 0.280 | 0.630 | -0.350 <- LARGEST |
  | T5| API/ROUTE | PHP   | 0.300 | 0.440 | -0.140 |
  | T6| BUGFIX    | Go    | 0.150 | 0.200 | -0.050 (11 files, tight) |
  | T7| API/ROUTE | JS    | 0.330 | 0.420 | -0.090 |
  Wins: T2 Python BUGFIX, T3 TS BUGFIX, T4 Python API/ROUTE, T8 TS DI BUGFIX
  (_CONTAINER_DI_RE still paying off 0.450 vs 0.000), T9 TS FEATURE, T10 Python
  BUGFIX. BUGFIX 4/6, API/ROUTE 1/3 (persistent weakness), FEATURE 1/1.

ROOT CAUSE per loss:
  * T1 (C++ BUGFIX, -0.350): no C++ specific strategy. King understands C++
    patterns (memory lifecycle, header interfaces) far better; we shipped a
    partial/incorrect patch (0.280). Real implementation gap, not polish noise.
  * T5 (PHP API/ROUTE) + T7 (JS API/ROUTE): API/ROUTE win rate only 33% (1/3).
    We spend steps exploring files before implementing while the king writes
    the core route immediately. On T7 our cursor-sim (0.121) actually BEAT the
    king's (0.055) yet we still lost -- different quality axis: completeness of
    the implemented route, not patch similarity.
  * T6 (Go BUGFIX, 11 files): both score low (0.150 vs 0.200); a sprawling
    11-file Go repo where we spread changes too thin instead of focusing on the
    one owning package.

EXACTLY THREE CHANGES vs Next33:
  * NEXT34 CHANGE 1 (C++ BUGFIX hint, fixes T1 -- HIGH PRIORITY): in
    `_integration_hints()`, add `_CPP_LANG_RE` (file ext .cpp/.cc/.c/.h/.hpp or
    'c++' mention) + `_BUGFIX_VERB_RE` (bug/fix/crash/leak/...). When BOTH fire,
    append a hint: read header files first; check memory lifecycle (RAII, raw
    ptr -> smart ptr, destructor), null/bounds guards, const-correctness, and
    header-declared vs defined discrepancies; fix the minimal set of files. See
    `_integration_hints` (C++ branch) and the `_CPP_LANG_RE`/`_BUGFIX_VERB_RE`
    regex defs above `_integration_hints`.
  * NEXT34 CHANGE 2 (API/ROUTE first-action hint, fixes T5 + T7): in
    `build_initial_user_prompt()`, inside the existing `_is_api_route_task()`
    branch (right after `_API_TASK_HINT`), append an implementation-order hint:
    write the core route/endpoint handler FIRST, then wire
    auth/middleware/validation, then update tests; do not read more than 2
    additional files before the first edit -- start implementing immediately.
    Pushes the agent to implement rather than explore. See
    `build_initial_user_prompt`.
  * NEXT34 CHANGE 3 (Go large-repo focus, fixes T6): in `_integration_hints()`,
    add `_GO_LANG_RE` (.go ext or 'golang'). When language=Go AND
    `_FILE_EXT_RE` count > 7, append a hint: identify the primary
    package/goroutine that owns the feature; focus changes on that package only;
    avoid integration test files unless the fix requires them. See
    `_integration_hints` (Go branch).

UNCHANGED (do NOT regress -- verbatim from Next33): _build_polish_task TEXT
(king-byte-identical, SHA 53bca97c), _polish_worth_adopting guard (Next33 fix),
_CONTAINER_DI_RE (T8 fix, still paying off 0.450 vs 0.000), render_observation
(king-identical <=3 ONLY), sampling params (temperature/max_tokens), _sanitize_patch,
_recovery_prompt, _is_large_repo_task, _is_api_route_task, _is_js_integration_task,
_API_TASK_HINT text. stdlib only; zero new imports.

DEFERRED (noted, NOT implemented -- would exceed the EXACTLY-3 budget): a PHP-
specific backend pattern hint (token gen -> hash store -> mail dispatch) for T5,
and a dedicated Go recovery-budget extension for T6. CHANGE 2 (generic API/ROUTE
first-action) already targets T5; CHANGE 3 already targets T6. Revisit only if
gate-34 still shows PHP/Go losses after these.
============================================================================
...
# NEXT34 CHANGE 1: C/C++ language + BUGFIX detectors. Gate-33 T1 (C++ Password
# Manager) was our LARGEST loss (0.280 vs king 0.630, -0.350). We have no C++
# specific strategy. `_CPP_LANG_RE` matches C/C++ via file extensions
# (.cpp/.cc/.c/.h/.hpp) or an explicit 'c++'/'C++' mention; `_BUGFIX_VERB_RE`
# matches bug/fix/issue/crash/leak/segfault phrasing (a true BUGFIX, not a new
# build). When both fire we inject a C++ header-first / memory-lifecycle hint.
...
# NEXT34 CHANGE 3: Go language detector for the large-repo focus branch. Gate-33
# T6 (P2P SkyFS Sync, 11 files, Go) lost 0.150 vs 0.200 on a sprawling 11-file
# Go repo. When the task is Go AND has >7 file-extension mentions, steer the
# agent to the primary owning package and away from integration test files.
...
def _integration_hints(issue: str) -> List[str]:
    ...
    # NEXT34 CHANGE 1 (C++ BUGFIX hint -- closes gate-33 T1, our LARGEST loss
    # 0.280 vs king 0.630): when the task is C/C++ (file ext or 'c++' mention)
    # AND it is a BUGFIX (bug/fix/crash/leak/etc.), point the agent at the
    # header files first and at the common C++ bug classes (memory lifecycle,
    # null/bounds, const-correctness, header decl vs def). We had no C++ specific
    # strategy; the king dominated this language.
    ...
    # NEXT34 CHANGE 3 (Go large-repo focus -- closes gate-33 T6, 0.150 vs 0.200
    # on an 11-file Go repo): when the task is Go AND mentions >7 file
    ...
    # of spreading thin across 11 files.
...
def build_initial_user_prompt(issue: str, repo_summary: str, preloaded_context: str = "") -> str:
    ...
    if _is_api_route_task(issue):
        ...
        # NEXT34 CHANGE 2 (API/ROUTE first-action hint -- closes gate-33 T5 PHP
        # 0.300 vs 0.440 + T7 JS 0.330 vs 0.420): our API/ROUTE win rate is only
        # 33% (1/3). On these multi-file integration tasks we spend steps
        # exploring files before implementing, while the king implements the
        # core route immediately. Push the agent to write the handler logic
        # FIRST, then wire auth/middleware/validation, capping pre-edit reading.
        prompt = prompt + (
            "\n[Implementation order: (1) write the core route/endpoint handler "
            "logic first, (2) then wire auth/middleware/validation, (3) then "
            "update tests if needed. Do not read more than 2 additional files "
            "before making your first edit -- start implementing immediately.]"
        )
