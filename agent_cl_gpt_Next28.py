...
def solve(
    ...
    try:
        ...
        if not outcome.patch.strip():
            ...
            if remaining >= 60:
                recovery_prompt = (
                    "The repository has no changes yet. "
                    "Apply the single most critical fix from the issue right now. "
                    "Read the most relevant source file, make the minimal necessary edit, "
                    "run a quick check, then submit.\n\nOriginal task:\n" + issue
                )
