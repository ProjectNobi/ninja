#!/usr/bin/env python3
"""Next44 -- base Next43 + REMOVE _PYTHON_COMPLETE_RE (caused 8 regressions)
+ keep expanded _LARGE_REPO_RE. stdlib only; zero new imports.

================================= NEXT44 (this version) =====================
G43 evidence: 7W-10L (41%) at 17 tasks -- KILLED at 17 ❌, a massive regression
from G42's 21W-8L (72%) ✅. ROOT CAUSE: `_PYTHON_COMPLETE_RE` fired on PHP/JS/TS
tasks too (their issue text mentions "python"), injecting a Python-completeness
hint that confused the agent and drove over-implementation -> churn penalty.
It produced 8 regressions (T3 TS, T5 PHP, T7 JS, T8 TS DI, T9 FEATURE TS,
T12 PHP, T13 Python OTHER, T15 UPDATE TS) vs only 3 fixes (T2/T11/T14),
net -5 rounds.

THE EXACTLY TWO CHANGES vs Next43:
  * NEXT44 CHANGE 1 -- REMOVE `_PYTHON_COMPLETE_RE` entirely. Delete the regex
    definition AND the branch in `_integration_hints()`. This restores G42's
    proven performance on T3/T5/T7/T8/T9/T12/T13/T15.
  * NEXT44 CHANGE 2 -- KEEP the expanded `_LARGE_REPO_RE` from Next43 (it helped
    T11: G42 LOSS 0.150 -> G43 WIN 0.320). NOT reverted -- kept exactly.

Expected: G42's 21W-8L restored + T11 fixed = ~22W-7L (73%+).

--- prior Next43 notes (for history) ---
Next43 had EXACTLY TWO CHANGES vs Next42:
  * NEXT43 CHANGE 1 (REVERTED in Next44) -- Python-completeness hint.
...
# NEXT44 CHANGE 1: `_PYTHON_COMPLETE_RE` REMOVED. In G43 it fired on the issue
# text of non-Python tasks too (issue descriptions for PHP/JS/TS tasks frequently
# mention "python"), injecting a Python-completeness hint that confused the agent
# and caused over-implementation -> churn. It produced 8 regressions (T3 TS, T5
# PHP, T7 JS, T8 TS DI, T9 FEATURE TS, T12 PHP, T13 Python OTHER, T15 UPDATE TS)
# vs only 3 fixes (T2/T11/T14), net -5 rounds. Both the regex definition and its
# branch in `_integration_hints()` are deleted, restoring G42 behavior.
...
def _integration_hints(issue: str) -> List[str]:
    ...
    # NEXT44 CHANGE 1: Python-completeness hint REMOVED (was Next43). It fired on
    # non-Python tasks whose issue text mentioned Python, causing 8 regressions.
    return hints
