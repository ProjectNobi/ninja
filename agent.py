...
DEFAULT_MAX_STEPS = int(os.environ.get("AGENT_MAX_STEPS", "18"))
...
BEFORE making any edits, first output a structured analysis:
ROOT_CAUSE: [the exact bug/missing feature at its origin]
REQUIRED_FILES: [every file that must change]
DOWNSTREAM_CALLERS: [functions/methods that call the code you will change]
Only after this analysis, begin your edits.
"""
...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    def maybe_queue_refinement(assistant_text: str) -> bool:
        ...
        """
        nonlocal polish_turns_used, self_check_turns_used, syntax_fix_turns_used, test_fix_turns_used, coverage_nudges_used, criteria_nudges_used, hail_mary_turns_used, wiring_check_turns_used, total_refinement_turns_used
        ...
        if wiring_check_turns_used < 1:
            wiring_check_turns_used += 1
            total_refinement_turns_used += 1
            queue_refinement_turn(
                assistant_text,
                "Review your patch. Are there callers, importers, or consumers of the code you changed that are NOT yet updated in your patch? If any remain, update them now.",
                "WIRING_CHECK_QUEUED",
            )
            return True
