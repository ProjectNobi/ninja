...
detector. Base is Next42 (2171 lines, best-ever 21W-8L/72% task-run-42/30). stdlib
only; zero new imports. EXACTLY 3 changes vs Next42.

================================= NEXT45 (this version) =====================
GATE-44 (Next42 + 2 tweaks) 10-task task-run-42 = 7W-3L (70%) FAIL. Win-by-type:
...
ever: 21W-8L (72%) task-run-42/30. This is the Next45 base.
...
================================= NEXT41 (this version) =====================
Gate evidence on the 30-task task-run: G37-30 3W-9L (25%) | G39-30 1W-4L-1T (17%) |
...
THE EXACTLY THREE CHANGES vs Next40:
  ...
  * NEXT41 CHANGE 3 -- REMOVE `_language_hints()` AND ITS REGEXES. The function
    ...
    narrow, task-run-volatile overfits (G40: C++ T1 LOSS, FEATURE T9 LOSS) and the
...
TASK_TEMPLATE (now king-equivalent) but TWO missing king mechanisms:
  1. The king's `_integration_hints()` uses 6 simple, broad, CONTENT-BASED
     ...
     precision, statically-typed, Go) that overfit the 10-task task-run and
...
to the fixed 10-task gate task-run: it passed gate-37 10-task but collapsed on the
...
EVIDENCE (Next37/38 vs king `hashirama`, SHA 53bca97c):
  * Gate-37 10-task: 8W-2L (80%) -- PASSED, but a lucky/overfit task-run.
  * Gate-37 30-task (killed at 12): 3W-9L (25%) -- STRUCTURAL FAILURE.
  * Gate-38 10-task: 4W-6L (40%).
  * BUGFIX win rate 16-67% depending on task-run -- structurally weak.
  * T8 TS DI: 0.750 (G36) -> 0.060 (G37-30) -- the "dominant" DI hint now HURTS
    ...
    win -> loss. Classic overfit-to-task-run signature.
ROOT CAUSE: overfitting to the 10-task task-run; our TASK_TEMPLATE was weaker than
...
THE THREE CHANGES vs Next37:
  ...
  * NEXT39 CHANGE 2 (hint pruning): remove the 6 most volatile/overfit hints and
    ...
    multiple task-runs: the C++ BUGFIX hint (`_CPP_LANG_RE`+`_BUGFIX_VERB_RE`,
