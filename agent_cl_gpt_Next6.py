#!/usr/bin/env python3
"""SN66 miner with verify-repair pass and targeted prompt improvements.

Adds a structured bug-fix workflow to the SYSTEM_PROMPT: locate the defect,
trace to root cause, make the minimum correct edit, and verify syntax before
submitting. All other solver behaviour (completeness rules, urgency hints,
empty-reply guard, repair loop) is preserved from the base.
"""
...
class ChatModel:
    ...
    def _extract_content(self, raw: str) -> str:
        ...
        # silently advancing on a no-op step (ported from Next1, upstream fix
        # ae2158103232). The base king submits whatever diff exists when the
        # model returns empty content; retrying lets the agent recover.
...
4. Minimal correct edit: change the minimum lines needed to fix the root cause.
   A focused fix beats a broad refactor. Only add logging or error handling if
   the task explicitly requires it — completeness means covering all bug criteria,
   not adding unrelated code.
...
def render_observation(
    *,
    ...
    # Graduated urgency hints at 5 / 3 / 1 remaining steps (ported from Next1).
    # The king only had a single `remaining_steps <= 3` note; this extends it
    # without removing the king's completeness-and-submit framing.
    notes = []
