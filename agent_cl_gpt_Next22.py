#!/usr/bin/env python3
"""SN66 Next22 challenger -- Next21 + TWO surgical changes (M2.7 design, Opus build).

Next22's two (and ONLY two) changes vs Next21:

  * CHANGE 1 (empty-diff bug fix in _sanitize_patch): T6 Go BUGFIX scored 0.000
    in the Next21 gate -- an EMPTY DIFF was submitted (guaranteed loss). Root
    cause: _sanitize_patch() could collapse a non-empty raw diff to empty/
    whitespace (false positive on the refusal/placeholder scrub). Fix: add a
    hard invariant -- if the input patch was non-empty but the sanitized result
    is empty/whitespace, return the ORIGINAL raw patch unchanged. The function
    now NEVER zeroes out a non-empty diff. A possibly-auto-failed real patch is
    always strictly better than an empty submission (a guaranteed 0).

  * CHANGE 2 (recalibrate _is_large_scope to a MEDIUM threshold): N21 required
    API keyword AND 2+ construction verbs and fired on only ~2/10 tasks (too
    narrow), removing the helpful planning turn from T4 ('Enhance Repository
    Analysis Backend with Full AI Pipeline' -- API keyword but only 1 verb) and
    T7, causing API regressions. N20 fired on ~7/10 (too broad -> BUGFIX
    regressions). N22 keeps the API keyword as a hard gate but loosens (b) to:
    API keyword AND (1+ construction verb OR 3+ substantive sentences). 'enhance'
    is added to the construction-verb set (only ever counts after an API keyword
    is confirmed, so no BUGFIX false positives). Expected firing rate ~4-5/10 --
    the medium dial between N20 and N21. All other Next21 behavior is unchanged.

-- Below: Next21's history, kept verbatim --

Next21 challenger -- Next20 + one surgical fix: tighten _is_large_scope().
...
Next20 gate: W5/L5 = 50% (FAILED). Root cause: planning turn too aggressive.
  API/ROUTE: 100% ✅ (planning turn works perfectly for large-scope tasks)
  BUGFIX: 33% ❌ (planning turn fired on simple bugfixes, wasted a step)
  Regression analysis:
    T6 Go BUGFIX: planning turn fired ("sync" + 3 build verbs), wasted step
    T10 Python BUGFIX: planning turn fired ("implement" in issue), wasted step
    T1/T8: king variance, not our fault
  Root cause: _is_large_scope() fired on ANY api keyword OR >= 3 build verbs.
  "fix", "ensure", "improve" counted as build verbs — too common in bugfixes.

Next21's ONE (and ONLY one) change vs Next20 (M2.7 design, T68Bot build):
  (All five Next20 changes kept. This is a precision tightening only.)

  * CHANGE 6 (tighten _is_large_scope): Rewrite scope detection to require BOTH:
    (a) At least ONE strong API/architecture keyword (route/endpoint/pipeline/
        auth/service/controller/middleware/webhook/REST/GraphQL/migration/ORM)
    (b) AND at least 2 distinct CONSTRUCTION verbs (create/add/implement/build/
        introduce/develop/set up/integrate/establish/register/configure/inject)
        — NOT fix/ensure/improve/handle/remove (too common in bugfixes)
    This keeps planning turn for genuine API builds (T4/T5-style) while removing
    it from simple bugfixes (T6/T10-style). Expected: BUGFIX 33% → ~80%,
    API/ROUTE stays 100%, overall ~90%.
    (M2.7 design, T68Bot implementation.)

All Next19+Next20 changes kept:

  * NEXT19-CHANGE 1 (collapse fix): Pre-submit checklist interception.
  ...
  * NEXT19-CHANGE 2 (completeness_check repair trigger): After the main loop, if the
  ...
  * NEXT19-CHANGE 3 (convergence framing for <=6 tier): Rewrite the <=6 step pressure
...
def render_observation(*, returncode: int, output_text: str, remaining_steps: int) -> str:
    # NEXT17 CHANGE 1: earlier + escalating step pressure. The Next16 gate timed
    # out on every large TypeScript task because the only nudge fired at <= 3
    # steps -- too late to stop exploring. Fire a "stop exploring, apply the fix"
    # note at <= 6, and turn the <= 3 note into a hard "submit NOW, read no more
    # files" directive so the model spends its last steps on the patch, not on
    # navigation.
    if remaining_steps <= 3:
        remaining_note = (
            f"[FINAL {remaining_steps} command(s). Submit NOW with "
            f"`echo {COMPLETION_SENTINEL}` after your patch -- do not read more "
            f"files. Make sure every requirement is handled in the diff you have.]"
        )
    elif remaining_steps <= 6:
        # NEXT19 CHANGE 3: convergence framing (was "stop exploring" abort signal).
        # Duel 6862 analysis: the abort framing caused premature partial submits
        # on complex multi-requirement tasks. Convergence framing keeps the timing
        # benefit while telling the agent WHAT to do, not what to stop.
        remaining_note = (
            f"[{remaining_steps} steps left -- enough to complete the fix. "
            f"Write your patch now, address every requirement in the checklist "
            f"above, wire all new symbols into call sites, then submit with "
...
# ============================================================
# criteria (acceptance-checklist injection) -- king verbatim + Next20 additions
# ============================================================

# NEXT20 CHANGE 4+5 / NEXT21 CHANGE 6: Large-scope / API-route detection.
# Used by: (4) planning turn injection, (5) API auto-criteria.
#
# NEXT21 CHANGE 6: Tightened _is_large_scope() to require BOTH conditions.
# Next20 was too aggressive: fired on ANY api keyword OR >= 3 build verbs,
# which included "fix", "ensure", "improve" -- causing false positives on simple
# BUGFIXes (T6 Go, T10 Python regressions). Now requires BOTH:
#   (a) Strong API/architecture keyword
#   (b) AND 2+ genuine construction verbs (not fix/ensure/improve)
_API_ROUTE_RE = re.compile(
    r"\b(route|endpoint|API|pipeline|authentication|auth(?:orization|entication)?"
    r"|service|controller|middleware|webhook|REST|GraphQL|migration|ORM|CRUD"
    r"|repository|FastAPI|Flask|Express|Django|Laravel|router|schema)\b",
    re.I,
)
# NEXT21: construction verbs -- excludes fix/ensure/improve/handle/remove.
# 'add', 'update', 'extend', and 'enhance' are included because they only count
# when condition (a) (API keyword) is already confirmed, making false positives
# impossible (NEXT22 added 'enhance' so T4-style 'Enhance ... Pipeline' fires).
_LARGE_SCOPE_VERBS_RE = re.compile(
    r"\b(create|implement|build|introduce|develop|(?:set\s+up)|integrate"
    r"|establish|register|configure|inject|mount|instantiate|add|update|extend"
    r"|enhance)\b",
    re.I,
)
_API_AUTO_CRITERIA: List[str] = [
    "Every endpoint described must be wired to the router/app with the correct HTTP method",
    "All new modules, classes, and functions must be imported where they are used -- no dead code",
    "The API must be reachable from the entry point (app.py / main.py / index.js / routes/*.php etc.)",
]


def _is_large_scope(issue: str) -> bool:
    """True when a task is a genuine API/architecture build with real scope.

    NEXT22 CHANGE 2 (recalibrate threshold to MEDIUM): N20 fired on ~7/10 tasks
    (too broad -> BUGFIX regressions); N21 required API keyword AND 2+ verbs and
    fired on ~2/10 (too narrow -> T4/T7 API regressions). N22 keeps the API
    keyword as a hard gate (simple BUGFIXes that mention no API/architecture term
    NEVER fire), but loosens condition (b) to fire on API keyword PLUS EITHER:
      - 1+ distinct construction verb (T4 'Enhance ... Pipeline' now fires), OR
      - 3+ substantive sentences (multi-step tasks without an explicit verb).
    Expected firing rate ~4-5/10 -- the medium dial between N20 and N21.

    FIRES:    'Enhance Repository Analysis Backend with Full AI Pipeline'  (API + 'Enhance')
    FIRES:    'Implement Password Reset Functionality. Build POST /reset endpoint,
               create ResetToken model.'  (auth/endpoint + Implement/Build/create)
    FIRES:    'Implement Intent Detection API and Update Chat Endpoint'  (API/endpoint + verbs)
    NO FIRE:  'Fix the P2P sync robustness and offline catch-up for SkyFS'  (no API keyword)
    NO FIRE:  'Improve Streamable HTTP Server Error Handling'  (no API keyword)
    NO FIRE:  'Enhance Password Manager Functionality'  (no API keyword)
    """
    text = issue or ""
    # Condition (a): strong API/architecture keyword required -- no API keyword,
    # never fire (keeps simple BUGFIXes out of the planning turn).
    if not _API_ROUTE_RE.search(text):
        return False
    # One construction verb is enough when we already have an API keyword.
    distinct_verbs = {m.lower().replace(" ", "") for m in _LARGE_SCOPE_VERBS_RE.findall(text)}
    if len(distinct_verbs) >= 1:
        return True
    # Fallback: multi-sentence task with an API keyword = large scope, even when
    # it names no explicit construction verb (clearly multi-step regardless).
    ...
    if len(out) < 2:
        for fallback in (
            ...
            if fallback not in out:
                out.append(fallback)
    # NEXT20 CHANGE 5: API auto-criteria injection.
    # For API/ROUTE tasks, automatically append 3 wiring criteria that the
    # pre-submit interception (Change 1) and completeness_check repair (Change 2)
    # will enforce. Zero cost on non-API tasks.
    if _is_large_scope(issue):
        for ac in _API_AUTO_CRITERIA:
            if ac not in out:
                out.append(ac)
    return out[:18]  # bumped from 15 to 18 to fit API auto-criteria
...
def _sanitize_patch(patch_text: str) -> str:
    ...
    NEXT22 CHANGE 1 (empty-diff bug fix): the absolute invariant is that this
    function NEVER zeroes out a non-empty patch. T6 Go BUGFIX scored 0.000
    because an empty diff was submitted -- a false positive where sanitizing a
    non-empty raw patch produced empty/whitespace output. Guard: if the input
    patch was non-empty but the sanitized result is empty or whitespace-only,
    return the ORIGINAL raw patch unchanged. A possibly-auto-failed real patch
    is always strictly better than an empty submission (guaranteed 0)."""
    try:
        ...
        if not kept_real_addition:
            return patch_text
        sanitized = "".join(out)
        # NEXT22 CHANGE 1: never return an empty/whitespace diff when the input
        # was non-empty. If sanitizing somehow collapsed the patch to nothing
        # real, fall open to the ORIGINAL raw patch -- an empty submission is a
        # guaranteed 0 (the T6 Go failure mode) and is always the worst outcome.
...
def run_agent_loop(*, config: AgentRunConfig, task: str, planning_turn: bool = False) -> AgentOutcome:
    model = ChatModel(
    ...
    # NEXT20 CHANGE 4: planning turn injection for large-scope / API tasks.
    # Add the plan as an ASSISTANT turn (pre-filled) right after the task, so
    # the agent's first real turn starts with full scope awareness.
    # We use a synthetic assistant "plan request" + user "please plan" pattern:
    # user sees the task → assistant is asked to plan → assistant produces file list
    # → coding loop starts. Implemented as an extra user message asking for a plan.
    if planning_turn:
        messages.append({
            "role": "user",
            "content": (
                "Before writing any code, briefly list every file you will create or modify "
                "and what each will contain (one line per file). Do NOT write any code yet -- "
                "just the file plan. Start with `PLAN:` then the file list."
            ),
        })
...
def solve(
    ...
    try:
        ...
        # NEXT20 CHANGE 4: planning turn for large-scope / API tasks.
        # Before the coding loop starts, inject a planning turn that forces the
        # agent to map the full implementation scope (all files to create/modify).
        # This prevents T4-style catastrophic scope misses where the agent dives
        # into one component and never surfaces to handle the rest of the pipeline.
        # Costs 1 step on large tasks, zero on simple bugfixes.
        run_config._planning_injected = _is_large_scope(issue)  # type: ignore[attr-defined]
        outcome = run_agent_loop(
            config=run_config,
            task=build_initial_user_prompt(issue, "", ""),
            planning_turn=_is_large_scope(issue),
        )
