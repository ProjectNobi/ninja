...
This revision additionally:
  - extends the verify-repair syntax gate to JavaScript via `node --check`
    (toolchain-only; degrades to a no-op when node is absent, with a hard
    JSX/Flow false-positive guard so valid React code is never re-rolled);
  - retries transient soft-empty/no-content 200-OK model replies in-place
    ...
    fresh-every-round agent stops losing rounds to empty responses; and
...
# Non-Python syntax check, TOOLCHAIN-ONLY. The base repair gate runs compile()
...
# needless re-solves and RAISE retest variance. No tool present -> no opinion
# -> identical to baseline. node is the only polyglot toolchain reliably
# present in the network-isolated sandbox, so we scope this to .js.
#
# JSX HARD GUARD (variance-safety): `node --check` is a plain V8 parser and
# CANNOT parse JSX/Flow. The live pool is JS/TS/React-dominant, so a perfectly
# VALID React patch trips node in two ways:
...
#       exactly the variance/latency injection we forbid. After these guards,
#       the gate only flags GENUINELY broken plain JS and degrades to a no-op
#       everywhere else, so it can only reduce variance, never add it.
_JS_EXTS = (".js", ".mjs", ".cjs")
...
def _changed_js_files(patch_text: str) -> list:
    """JavaScript files touched by the patch (parsed from its `+++ b/` headers)."""
    paths = []
    for line in patch_text.splitlines():
        if line.startswith("+++ b/"):
...
def _js_syntax_errors(repo_dir: str, patch_text: str) -> list:
    ...
    Returns [] when node is absent so the gate degrades to the baseline."""
