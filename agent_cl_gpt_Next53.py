...
def solve(
    ...
    try:
        ...
        if not outcome.patch.strip():
            ...
            # at least one full LLM round-trip plus the enlarged 30s reserve.
            if remaining >= 120:
                ...
                recovery_config = AgentRunConfig(
                    ...
                    max_observation_chars=MAX_OBSERVATION_CHARS,
                    max_log_chars=MAX_TOTAL_LOG_CHARS,
                    # NEXT53 CHANGE 1: reserve raised 10 -> 30 (ported from
                    # Next52). The child loop stops earlier relative to the
                    # harness wall, so an in-flight LLM call (up to 180s) is far
                    # less likely to push total elapsed past the 300s harness
                    # limit.
            ...
            if remaining >= 150:
                ...
                if reason is not None and reason[0] in ("empty", "syntax", "corruption"):
                    ...
                    recovery_config = AgentRunConfig(
                        ...
                        max_observation_chars=MAX_OBSERVATION_CHARS,
                        max_log_chars=MAX_TOTAL_LOG_CHARS,
                        # NEXT53 CHANGE 1: reserve raised 10 -> 30 (ported from Next52).
                        wall_clock_limit=remaining - 30.0,
                        issue_text=issue,
                    )
                    recovered = run_agent_loop(config=recovery_config, task=build_initial_user_prompt(recovery_prompt, "", ""))
